...
const ResourceAdmin: React.FC = () => {
  ...
  const load = async () => {
    setLoading(true);
    try { const [r, q] = await Promise.all([resourceApi.listAdmin(), resourceApi.listRequests()]); setItems(r); setRequests(q); }
  ...
  const submit = async () => {
    ...
    try {
      ...
      setShowForm(false); setForm({ title: '', description: '', resourceType: 'project', tags: '', techStack: '', difficulty: '', targetRole: '', visibility: 'portal' }); setFiles(null);
      await load();
        ...
        <div style={{ overflowX: 'auto', border: '1px solid #e6e8f0', borderRadius: 12 }}>
          <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13.5, minWidth: 720 }}>
            <thead><tr style={{ background: '#f8fafc', textAlign: 'left' }}>
              {['Resource', 'Type', 'Visibility', 'Status', 'Files', 'Downloads', 'Requests', ''].map(h => <th key={h} style={{ padding: '10px 12px', fontSize: 11, textTransform: 'uppercase', color: '#64748b', letterSpacing: '.05em' }}>{h}</th>)}
            </tr></thead>
            <tbody>
              {items.map(r => (
              ...
              {items.length === 0 && <tr><td colSpan={8} style={{ padding: 24, color: '#94a3b8', textAlign: 'center' }}>No resources yet. Upload one to get started.</td></tr>}
            </tbody>
          </table>
        </div>
      ) : (
