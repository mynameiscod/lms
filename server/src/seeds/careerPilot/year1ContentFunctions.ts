...
without using "and"? \`calculate_total\` is a function. \`validate_and_save_and_email\` is three.`,
    mcqs: [
      mcq('What is the strongest reason to extract a repeated calculation into a function?',
        [['There becomes ONE place where that logic lives, so a correction cannot be half-applied', true],
          ['It saves typing', false],
      ...
      mcq('`interest = simple_interest(principal, rate, years)` is better than the raw formula mainly because:',
        [['The name puts the intent in the code rather than in a comment that can drift', true],
          ['It is shorter', false],
          ['It is faster', false],
          ['It avoids variables', false]],
    ...
    checkpoint: [
      mcq('A three-line calculation appears in four places. What is the real cost of leaving it there?',
        [['Four places must be corrected together, and one will be missed', true],
          ['Only extra typing', false],
          ['Slower execution', false],
          ['More memory used', false]],
      ...
      mcq('You cannot name a block of code without using the word "and". What does that tell you?',
        [['It is doing more than one job and should be split', true],
          ['The name should be longer', false],
          ['It needs a comment', false],
    ...
    mcqs: [
      ...
      mcq('`greet` without parentheses evaluates to:',
        [['The function object itself, not its result', true],
          ['The result of calling it', false],
      ...
      mcq('Why does calling a function above its `def` raise NameError?',
        [['Python executes top to bottom, so the name does not exist yet at that point', true],
          ['Functions must be in alphabetical order', false],
          ['The call needs parentheses', false],
          ['It does not; that works', false]],
    ...
    checkpoint: [
      mcq('A file has a call placed ABOVE the definition it calls. What happens?',
        [['NameError - the name does not exist yet when the call is reached', true],
          ['It works; Python scans the whole file first', false],
          ['A SyntaxError', false],
          ['The function runs twice', false]],
      ...
      mcq('print(greet) outputs "<function greet at 0x...>". What was forgotten?',
        [['The parentheses - the function was printed rather than called', true],
          ['A return statement', false],
          ['An argument', false],
          ['The def keyword', false]],
...
and knowing which kind you have written is part of writing it.`,
    mcqs: [
      ...
      mcq('`percentage(50, 45)` where the definition is `percentage(scored, total)` gives 111.1. Why no error?',
        [['Both are numbers, so the division is valid — the values were simply swapped', true],
          ['There is an error, it is just hidden', false],
          ['Python cannot check argument order', false],
          ['It returns None', false]],
      ...
      mcq('A parameter named `total` inside a function and a variable named `total` outside it are:',
        [['Unrelated — the parameter is local and disappears when the function returns', true],
          ['The same variable', false],
          ['A name collision error', false],
          ['Linked only if the same value is passed', false]],
      ...
      mcq('Why is `create_user("Asha", 20, True, False, True)` a problem?',
        [['Nothing at the call site says what the three booleans mean', true],
          ['Five arguments is a syntax error', false],
          ['Booleans cannot be arguments', false],
          ['It will be slow', false]],
    ...
    checkpoint: [
      mcq('percentage(scored, total) called as percentage(50, 45) returns 111.1 and raises nothing. Why?',
        [['Both arguments are valid numbers, so only the ORDER was wrong and nothing can detect that', true],
          ['Python cannot divide those numbers', false],
          ['The function is missing a guard', false],
          ['There is an error, and it is suppressed', false]],
      ...
      mcq('A parameter named total and an outer variable named total are related how?',
        [['Not at all - the parameter is local and vanishes when the function returns', true],
          ['They are the same variable', false],
          ['The parameter shadows it permanently', false],
          ['Python raises a warning', false]],
...
variable and print that — \`None\` tells you immediately.`,
    mcqs: [
      mcq('`def f(n): print(n * 2)` — what is `x` after `x = f(5)`?',
        [['None, because the function printed rather than returned', true],
          ['10', false], ['5', false], ['An error', false]],
      ...
      mcq('Why does the missing-return bug usually surface far from its cause?',
        [['The function itself succeeds; the None only fails when something tries to USE it', true],
          ['Python delays errors', false],
          ['print is asynchronous', false],
          ['It does not', false]],
      ...
      mcq('`def stats(n): return min(n), max(n)` returns:',
        [['One tuple, which the caller can unpack into two names', true],
          ['Two separate values', false],
          ['A list', false],
          ['Only the minimum', false]],
    ...
    checkpoint: [
      mcq('A function prints its result instead of returning it. Where does the error appear?',
        [['Wherever something first tries to USE the value, possibly several lines away', true],
          ['On the print line', false],
          ['Immediately, at the call', false],
          ['At the def line', false]],
...
Use \`None\` as the default for any mutable value — list, dict, set.`,
    mcqs: [
      mcq('Why is `def f(a=1, b)` a SyntaxError?',
        [['With `f(5)` Python could not tell whether 5 was `a` or `b`', true],
          ['Defaults are not allowed in Python', false],
          ['`a` must be a string', false],
          ['Only one default is permitted', false]],
      ...
      mcq('`def add(item, basket=[])` returns a growing list across separate calls. Why?',
        [['The default list is created once at definition time and shared by every call', true],
          ['Lists are global', false],
          ['append mutates the argument', false],
      ...
      mcq('The correct fix for a mutable default is:',
        [['Default to None and create the value inside the function', true],
          ['Use a tuple instead', false],
          ['Copy the list at the end', false],
          ['Make it a global', false]],
      ...
      mcq('When is naming arguments at the call site most valuable?',
        [['When there are several, or any of them are booleans', true],
          ['Always, without exception', false],
          ['Only for defaults', false],
          ['Never; position is clearer', false]],
    ...
    checkpoint: [
      mcq('def add(item, basket=[]) accumulates across separate calls. The cause is:',
        [['The default is evaluated once at definition time, so every caller shares one list', true],
          ['append modifies the list belonging to the caller', false],
          ['Lists are always global', false],
          ['Python caches return values', false]],
      ...
      mcq('Why must parameters carrying defaults come last?',
        [['Otherwise a single positional argument could not be matched unambiguously', true],
          ['Readability only', false],
          ['It is an arbitrary rule', false],
          ['To allow named arguments', false]],
...
the line that calls it. It is also the version you can test.`,
    mcqs: [
      mcq('Why does `count = count + 1` inside a function raise UnboundLocalError when `count` exists outside?',
        [['The assignment makes `count` local for the whole body, so the read finds an unassigned local', true],
          ['The global is read-only', false],
          ['`count` must be declared first', false],
      ...
      mcq('A function READING a module-level constant is:',
        [['Fine — reading an outer name needs no declaration', true],
          ['An error', false],
          ['Only allowed with `global`', false],
          ['Bad practice in all cases', false]],
      ...
      mcq('Why prefer passing and returning over `global`?',
        [['The effect becomes visible at the call site rather than happening invisibly elsewhere', true],
          ['It is faster', false],
          ['`global` is deprecated', false],
      ...
      mcq('Local scope is described as a feature because:',
        [['You can name things inside a function without knowing what the rest of the program uses', true],
          ['It saves memory', false],
          ['It makes functions faster', false],
          ['It prevents typos', false]],
    ...
    checkpoint: [
      mcq('count = count + 1 inside a function raises UnboundLocalError though count exists outside. Why?',
        [['Assigning anywhere in the body makes the name local for the WHOLE body', true],
          ['The outer variable is private', false],
          ['Integers cannot be reassigned', false],
          ['count is a reserved word', false]],
      ...
      mcq('The preferred fix for that is:',
        [['Pass the value in and return the new one, so the effect is visible at the call site', true],
          ['Declare it global', false],
          ['Rename the outer variable', false],
          ['Initialise it inside the function', false]],
...
are for performance, they have the wrong reason.`,
    mcqs: [
      mcq('The main benefit of splitting one long function into four is:',
        [['Each piece can be checked on its own, and a change is confined to one of them', true],
          ['The program runs faster', false],
          ['It uses less memory', false],
          ['It is shorter overall', false]],
      ...
      mcq('When grade boundaries change in the composed version, what must you inspect?',
        [['Only the `grade` function, because nothing else mentions boundaries', true],
          ['Every function', false],
          ['The whole file', false],
          ['`report` and `grade`', false]],
      ...
      mcq('Why does a function that reads a global lose the "check it separately" property?',
        [['Its result depends on state you must set up first, so it is not independent', true],
          ['Globals are slow', false],
          ['It cannot be called twice', false],
          ['It will raise an error', false]],
      ...
      mcq('Which indicates composition has gone too far?',
        [['A chain of four or five calls you must hold in your head to follow one operation', true],
          ['Having more than three functions', false],
          ['Any function under five lines', false],
          ['Functions calling functions at all', false]],
    ...
    checkpoint: [
      mcq('Splitting one long function into four buys which property?',
        [['Each piece can be verified alone, and a change is confined to one of them', true],
          ['Faster execution', false],
          ['Shorter code overall', false],
          ['Less memory', false]],
      ...
      mcq('Which function in a composed chain is hardest to test, and why?',
        [['One that reads a global, because it depends on state you must arrange first', true],
          ['The longest one', false],
          ['The one called most often', false],
          ['The top-level one', false]],
    ...
    mcqs: [
      mcq('Why is a docstring better than a `#` comment above the function?',
        [['It is stored by Python, so help() and editors can show it', true],
          ['Comments are not allowed there', false],
          ['It is shorter', false],
          ['Comments slow the program', false]],
      ...
      mcq('Why does stating the unit of a `rate` parameter matter?',
        [['7.5 and 0.075 both look plausible and only one is right — the caller cannot tell', true],
          ['It is required by PEP 8', false],
          ['It makes the function faster', false],
          ['Python checks it', false]],
      ...
      mcq('The test for a finished docstring is:',
        [['Somebody could use the function correctly, edge cases included, without reading the body', true],
          ['It is at least three lines', false],
          ['It mentions every parameter', false],
          ['It explains the algorithm', false]],
    ...
    checkpoint: [
      mcq('Which fact belongs in a docstring because the signature cannot show it?',
        [['Whether a rate parameter means 7.5 or 0.075', true],
          ...
          ['That it is a function', false]],
        'Units and conventions are invisible from the signature and are a recurring source of silently wrong results.'),
      mcq('The test for a finished docstring is:',
        [['A caller could use it correctly, edge cases included, without reading the body', true],
          ['It mentions every parameter', false],
          ['It is at least two lines long', false],
          ['It explains the algorithm', false]],
        'Saving the reader from the body is the entire purpose. Explaining the algorithm is usually a job for a comment inside it.'),
    ],
  },
  {
...
print(report(a, b, c))`,
    ...
    mcqs: [
      mcq('`TypeError: unsupported operand type(s) for +: \'NoneType\' and \'int\'`. Where is the bug?',
        [['In whichever function printed instead of returning — not on the line that failed', true],
          ['On the line with the +', false],
          ['In the input parsing', false],
          ['In the function signature', false]],
      ...
      mcq('`UnboundLocalError` for a name that clearly exists above the function means:',
        [['You assigned to it inside, making it local for the entire body', true],
          ['The variable is misspelt', false],
          ['The function needs `global`', false],
      ...
      mcq('Why print both the arguments and the return value when debugging a function?',
        [['It separates "this function is wrong" from "it is being given the wrong thing"', true],
          ['It is faster than a debugger', false],
          ['It prevents type errors', false],
          ['Two prints are conventional', false]],
      ...
      mcq('Why use `!r` in a debug print?',
        [['It shows quotes, so the string "5" is distinguishable from the number 5', true],
          ['It is shorter', false],
          ['It prints faster', false],
          ['It adds the type name', false]],
...
have only learned what it does.`,
    mcqs: [
      mcq('`def f(items): return len(items) / 2` — what should you check before trusting it?',
        [['What it does with an empty list, and whether you wanted integer or float division', true],
          ['Whether items is a list', false],
          ['That it has a docstring', false],
          ['Nothing; it is correct', false]],
      ...
      mcq('A function takes four parameters and uses two. What does that suggest?',
        [['The signature is wrong — it is asking callers for things it does not need', true],
          ['Nothing; extra parameters are harmless', false],
          ['It needs a default', false],
          ['It should be split', false]],
      ...
      mcq('`def tax(amount, rate=0.18)` called as `tax(100, 18)` returns 1800. The bug is:',
        [['The caller used a percentage where the default implies a fraction', true],
          ['The default is wrong', false],
          ['Two arguments are not allowed', false],
          ['`rate` should be a string', false]],
      ...
      mcq('Why trace a function on paper before running it?',
        [['Running tells you what it does; predicting tells you whether your model of it is right', true],
          ['It is faster', false],
          ['It finds syntax errors', false],
