...
topic is about *looking* rather than about reading errors.`,
    mcqs: [
      mcq('Why prefer an external stylesheet over inline styles?',
        [['It applies across pages, is cacheable, and is not hardest to override', true],
          ['Inline styles are not valid HTML in a modern document', false],
          ['An external file always loads faster than inline rules', false],
          ['Inline styles cannot make use of classes at all', false]],
      ...
      mcq('You misspell a property name. What does the browser report?',
        [['Nothing — the declaration is silently skipped', true],
          ['A console error naming the property it did not know', false],
          ['The page fails to render until the typo is corrected', false],
          ['A warning in the network tab beside the stylesheet', false]],
      ...
      mcq('The point of separating structure from presentation is mainly that:',
        [['The same content can be presented differently by different consumers', true],
          ['The stylesheet ends up smaller than it would be', false],
          ['The HTML loads faster with the styling removed', false],
          ['Browsers require the two to be kept in separate files', false]],
    ...
    checkpoint: [
      mcq('A rule appears correct and has no effect at all. Which is NOT a plausible cause?',
        [['The browser has reported a CSS syntax error you missed', true],
          ['The selector does not match any element on the page', false],
          ['Another, more specific rule is winning the conflict', false],
    ...
    mcqs: [
      mcq('`.card.wide` selects:',
        [['One element carrying BOTH classes', true],
          ['An element with class wide inside one with class card', false],
          ['Any element carrying either of the two classes', false],
          ['Nothing at all, because the syntax is not valid', false]],
      ...
      mcq('Why prefer classes over ids for styling?',
        [['Ids carry high specificity, so the rules resist overriding', true],
          ['Ids have been deprecated for use in CSS selectors', false],
          ['Classes are faster for the browser to match', false],
          ['An id cannot be used as a selector in a stylesheet', false]],
      ...
      mcq('`.card > p` differs from `.card p` how?',
        [['The first matches only direct children; the second matches any depth', true],
          ['They are identical; the combinator is optional', false],
          ['The first is faster, since the search stops sooner', false],
          ['The second matches only the first paragraph inside', false]],
    ...
    checkpoint: [
      mcq('A rule has no effect. What is the fastest first check?',
        [['Inspect the element and see which rules the browser says apply', true],
          ['Re-read the stylesheet from the top, looking for the conflict', false],
          ['Add `!important`, which settles the question of which rule wins', false],
          ['Restart the browser, in case the stylesheet has been cached', false]],
    ...
    mcqs: [
      mcq('`#nav a` (1,0,1) versus `.menu .link .active` (0,3,0). Which wins?',
        [['`#nav a`, because one id beats any number of classes', true],
          ['`.menu .link .active`, since three classes beat one id', false],
          ['Whichever of the two appears last in the stylesheet', false],
          ['They tie, and the browser picks one arbitrarily', false]],
      ...
      mcq('When does source order actually decide the winner?',
        [['Only when specificity ties exactly', true],
          ['Always; the last matching rule in the file wins', false],
          ['Only when one of the declarations is marked important', false],
          ['Never; specificity decides every conflict on its own', false]],
      ...
      mcq('Why is `!important` a trap rather than a fix?',
        [['It removes any normal way to override it, escalating the next conflict', true],
          ['It does not work reliably across the major browsers', false],
          ['It is invalid CSS and will fail the validator', false],
          ['It slows rendering, since the cascade is recomputed', false]],
      ...
      mcq('An inherited `color` versus a rule setting `color` on the element directly:',
        [['The direct rule wins, however unspecific it is', true],
          ['Whichever of the two selectors is the more specific', false],
          ['The inherited value wins, since it came from further up', false],
          ['The two colours blend into an intermediate shade', false]],
    ...
    checkpoint: [
      ...
      mcq('Your rule loses to an id-based rule. The best fix is:',
        [['Add specificity honestly, for example by including the id in your selector', true],
          ['Add `!important`, which settles this conflict at once', false],
          ['Move your rule to the very bottom of the stylesheet', false],
          ['Use an inline style, which outranks anything in a stylesheet', false]],
    ...
    mcqs: [
      ...
      mcq('`box-sizing: border-box` changes things how?',
        [['Width includes padding and border, so padding eats inward', true],
          ['The margin is counted in the declared width as well', false],
          ['Padding is ignored entirely when the width is set', false],
          ['The border is removed from the rendered element', false]],
      ...
      mcq('`margin-bottom: 30px` meets `margin-top: 20px`. The gap is:',
        [['30px — vertical margins collapse to the larger', true],
          ['50px, since the two margins are added together', false], ['20px, because the smaller of the two is used', false], ['25px, the average of the two margin values', false]],
      ...
      mcq('A button needs breathing room around its text. Padding or margin?',
        [['Padding: it is inside the element, clickable, and takes the background', true],
          ['Margin, so the button size is left unchanged', false],
          ['Either one; there is no difference for a button', false],
          ['A transparent border of the required thickness', false]],
    ...
    checkpoint: [
      mcq('Three 300px cards with 16px padding and 1px borders overflow a 900px row. Best fix?',
        [['Set box-sizing: border-box globally', true],
          ['Reduce each card to 266px so that the row fits', false],
          ['Remove the padding from each of the three cards', false],
          ['Widen the row until the three cards fit inside', false]],
      ...
      mcq('Horizontal margins between flex children:',
        [['Do not collapse — and `gap` is the better tool inside a flex container', true],
          ['Collapse into one another, exactly as vertical margins do', false],
          ['Are ignored entirely, because flex controls the spacing itself', false],
          ['Double up, so the space between two children is twice the margin', false]],
    ...
    mcqs: [
      mcq('Why is `line-height: 1.5` preferred over `line-height: 24px`?',
        [['Unitless scales with each element\'s own font size; a fixed value does not', true],
          ['It is shorter to type and therefore easier to maintain', false],
          ['Pixel values are deprecated for anything to do with text', false],
          ['They are identical at the default root font size', false]],
      ...
      mcq('`#777` text on white is 4.48:1. What does that mean?',
        [['It fails WCAG AA for normal text, which needs 4.5:1', true],
          ['It passes comfortably for text of any size at all', false],
          ['Contrast requirements do not apply to shades of grey', false],
          ['It passes for large text, which needs only 3:1', false]],
      ...
      mcq('A form marks invalid fields with a red border only. What is missing?',
        [['Text or an icon; colour alone excludes anybody not seeing it', true],
          ['A thicker border, so the marked field stands out more', false],
          ['A brighter red, which is easier to pick out at a glance', false],
          ['Nothing; a red border is the established convention', false]],
    ...
    checkpoint: [
      ...
      mcq('Why define colours as custom properties on `:root`?',
        [['One place to change them, and the names say what they are for', true],
          ['They render faster, because the value is resolved only once', false],
          ['Hex codes are deprecated and custom properties replace them', false],
          ['It is required for dark mode, which cannot work without them', false]],
    ...
    mcqs: [
      mcq('With `flex-direction: column`, `justify-content` controls:',
        [['Vertical distribution, because the main axis is now vertical', true],
          ['Horizontal distribution, which is the main axis', false],
          ['Nothing; it only applies to a row direction', false],
          ['Wrapping, deciding when items move to a new line', false]],
      ...
      mcq('`flex: 1` on every child produces:',
        [['Equal widths regardless of content, because the basis is 0', true],
          ['Widths in proportion to how much content each holds', false],
          ['No change at all from the default flex behaviour', false],
          ['Items that grow but refuse to shrink below content', false]],
      ...
      mcq('You need a header, a sidebar and content whose rows and columns line up. Use:',
        [['Grid — it is a two-dimensional problem', true],
          ['Nested flex containers, one row inside a column', false],
          ['Flex with wrapping turned on for the container', false],
          ['Absolute positioning for each of the three regions', false]],
      ...
      mcq('Why use `gap` rather than margins between flex items?',
        [['It spaces children without a stray margin on the first or last', true],
          ['Margins have no effect inside a flex container', false],
          ['`gap` is faster for the browser to compute', false],
          ['Margins collapse inside a flex container as they do elsewhere', false]],
    ...
    checkpoint: [
      ...
      mcq('`align-items: center` on a default flex row centres items:',
        [['Vertically, because the cross axis is vertical', true],
          ['Horizontally, along the length of the row', false],
          ['In both directions at once within the container', false],
          ['Only when all the items are of equal height', false]],
...
awkwardness as using flex for everything.`,
    mcqs: [
      mcq('`repeat(auto-fit, minmax(250px, 1fr))` does what?',
        [['Fits as many 250px-minimum columns as will fit, with no media query', true],
          ['Creates columns of exactly 250px, however many fit', false],
          ['Creates one column for each item in the container', false],
          ['Reflows only when a media query changes the count', false]],
      ...
      mcq('`1fr` means:',
        [['One fraction of the space remaining after fixed tracks', true],
          ['One pixel of the track, which then repeats', false],
          ['One hundred per cent of the container width', false],
          ['One column, whatever width it turns out to be', false]],
      ...
      mcq('The advantage of `grid-template-areas` is:',
        [['The CSS contains a readable picture of the layout', true],
          ['It is faster for the browser to lay out than tracks', false],
          ['It works in more browsers than the track syntax', false],
          ['It avoids needing a class on each child element', false]],
      ...
      mcq('When is flex the better choice over grid?',
        [['Distributing items along one line where sizes follow their content', true],
          ['Any layout that has more than one row in it', false],
          ['Page-level layout, where grid is too heavy a tool', false],
          ['Whenever the container holds more than three items', false]],
    ...
    checkpoint: [
      mcq('A card grid must show 4 columns on desktop and 1 on a phone. Minimum CSS?',
        [['One grid with repeat(auto-fit, minmax(...)) — no media query needed', true],
          ['Three media queries, one for each of the layouts in between', false],
          ['Flex with wrap and fixed widths, which reflows at any size', false],
          ['A media query for every breakpoint the design calls for', false]],
      ...
      mcq('You need a page header, sidebar and main content to align across rows. Use:',
        [['Grid, because the alignment between rows is the requirement', true],
          ['A flex column holding flex rows, one per section', false],
          ['Floats, which is how this was done originally', false],
          ['Absolute positioning of each of the three regions', false]],
    ...
    mcqs: [
      mcq('`position: absolute` positions against:',
        [['The nearest positioned ancestor, or the page if there is none', true],
          ['The page itself, whatever the ancestors are doing', false],
          ['The immediate parent, whether positioned or not', false],
          ['The viewport, so it moves as the page is scrolled', false]],
      ...
      mcq('"Removed from the flow" means:',
        [['Others lay out as though it is absent, so it overlaps and the parent stays small', true],
          ['It is invisible, though it still occupies its space', false],
          ['It can no longer be styled by the ordinary rules', false],
          ['It renders last, after everything else is in place', false]],
      ...
      mcq('`z-index: 999` on a `position: static` element does what?',
        [['Nothing — z-index applies only to positioned elements', true],
          ['Puts it above everything else on the whole page', false],
          ['Causes an error the browser reports in the console', false],
          ['Works, but only inside a flex or grid container', false]],
      ...
      mcq('`position: sticky` with no `top` value does:',
        [['Nothing — it needs a threshold to stick at', true],
          ['Sticks to the top of the viewport by default', false],
          ['Behaves exactly as `position: fixed` would', false],
          ['Throws an error that the console will report', false]],
    ...
    checkpoint: [
      mcq('You need a badge in the top-right corner of a card. What is required?',
        [['`position: relative` on the card and `position: absolute` on the badge', true],
          ['`position: absolute` on the badge alone, with nothing on the card', false],
          ['`position: fixed` on the badge, so it stays with the card as it moves', false],
          ['`float: right` on the badge, which pushes it to the top right', false]],
      ...
      mcq('Most modern layouts need positioning for:',
        [['Badges, tooltips, dropdowns and overlays — things layered above content', true],
          ['All multi-column layouts, since columns have to be placed by hand', false],
          ['Centring elements, which is what absolute positioning is mainly for', false],
          ['Equal-height columns, which no other layout mechanism can produce', false]],
...
rest.`,
    mcqs: [
      mcq('Your media queries have no effect on a phone. The most likely cause is:',
        [['The viewport meta tag is missing, so the phone renders wide and scales', true],
          ['The queries use `min-width` where phones need `max-width`', false],
          ['The stylesheet has been cached from an earlier version', false],
          ['Phone browsers ignore media queries below a certain width', false]],
      ...
      mcq('Why prefer mobile-first `min-width` queries?',
        [['The base case is the constrained one, easiest to get wrong if left last', true],
          ['They are faster for the browser to evaluate', false],
          ['The `max-width` form has been deprecated in CSS', false],
          ['They always need fewer rules than the other direction', false]],
      ...
      mcq('Breakpoints should be chosen by:',
        [['Widening the browser until the layout looks wrong', true],
          ['The widths of the most common devices in use', false],
          ['Round numbers, which are easier to remember', false],
          ['The artboard widths the designer happened to use', false]],
    ...
    checkpoint: [
      mcq('A wide table makes the whole page scroll sideways. The fix is:',
        [['Wrap it in a container with overflow-x: auto so the table scrolls itself', true],
          ['Reduce the font size in the table until the columns fit the screen', false],
          ['Hide the less important columns below a mobile breakpoint', false],
          ['Set the table to `width: 100%`, which constrains it to its parent', false]],
      ...
      mcq('What is a sensible narrow width to test at?',
        [['360px — narrower than most phones', true],
          ['768px, which is the width of a common tablet', false], ['320px, the narrowest screen anybody still uses', false], ['1024px, since anything wider is a desktop', false]],
    ...
    mcqs: [
      mcq('Your rule appears in the Styles panel with a line through it. That means:',
        [['It matched, and a more specific rule won', true],
          ['The syntax of the declaration is invalid somewhere', false],
          ['The selector does not match the element at all', false],
          ['The property is not supported in this browser', false]],
      ...
      mcq('`width: 300px` on an inline element does nothing. Why?',
        [['Width does not apply to inline elements; the rule is inapplicable', true],
          ['The value is wrong and needs a unit after the number', false],
          ['It needs `!important` to beat the browser default', false],
          ['Inline elements cannot be styled with CSS at all', false]],
      ...
      mcq('Why `* { outline: 1px solid red }` rather than a border for debugging?',
        [['Outline does not affect layout, so the sizes stay as they were', true],
          ['An outline is more visible against a busy background', false],
          ['Borders are slower for the browser to paint', false],
          ['Outline applies to elements a border cannot reach', false]],
      ...
      mcq('Why change one declaration at a time?',
        [['CSS gives no attribution, so four changes leave four candidates', true],
          ['It is faster than reloading after every single edit', false],
          ['Browsers cache the stylesheet if too much changes', false],
          ['To avoid introducing a specificity conflict by accident', false]],
...
`,
    ...
    checkpoint: [
      mcq('Which panel tells you where an unexplained value came from?',
        [['Computed, which shows the final value and the rule that produced it', true],
          ['Styles, which lists every rule that could apply to the element', false], ['Network, which shows the stylesheet each value was loaded from', false], ['Console, where the browser logs any rule it could not resolve', false]],
      ...
      mcq('A page scrolls sideways and you cannot find the cause. A workable method is:',
        [['Hide halves of the page until the overflow goes, narrowing it down', true],
          ['Set `overflow: hidden` on the body, which removes the scrollbar', false],
          ['Reduce every width by ten per cent until the page fits again', false],
          ['Remove all margins, since a margin is what pushes content wide', false]],
  ...
  mock-up, and a layout that only works with the placeholder text is not finished.`,
    mcqs: [
      mcq('Working from a screenshot, what comes before any CSS?',
        [['Identify the box structure and write the unstyled HTML', true],
          ['Pick the colours and set up the custom properties', false],
          ['Set up the grid the layout is going to sit inside', false],
          ['Match the fonts against the ones in the screenshot', false]],
      ...
      mcq('Your layout works with the mock-up text and breaks with real content. What was missed?',
        [['Testing with content twice as long', true],
          ['A media query for the narrower screen widths', false],
          ['A global `box-sizing: border-box` declaration', false],
          ['A font fallback for when the web font fails', false]],
      ...
      mcq('Which global reset is worth applying on every project?',
        [['box-sizing: border-box and img { max-width: 100% }', true],
          ['Removing every default margin from every element', false],
          ['Setting every font size in pixels for consistency', false],
          ['Putting `position: relative` on every container', false]],
      ...
      mcq('Before calling a layout finished, test at:',
        [['360px, with the Tab key, and with doubled content', true],
          ['1920px, which is the commonest desktop width', false],
          ['The width the design was originally drawn at', false],
          ['The two breakpoints the stylesheet declares', false]],
