    ...
    mcqs: [
      mcq('The relationship between the three terms is:',
        [['AI contains machine learning, which contains deep learning', true],
          ['The three terms are synonyms used interchangeably', false],
          ['Deep learning is the widest of the three terms', false],
          ['They are three unrelated fields of computing', false]],
      ...
      mcq('The defining difference between rule-based and learned systems is:',
        [['A person writes the logic, versus the system deriving it from examples', true],
          ['Speed', false], ['The cost of building and running each of them', false], ['The programming language each is written in', false]],
      ...
      mcq('What do you give up by using a learned system?',
        [['The ability to say exactly why a particular decision was made', true],
          ['Accuracy, which a written rule can always beat', false], ['Speed', false], ['Nothing at all; a learned system is strictly better', false]],
      ...
      mcq('When is a rule-based system the better choice?',
        [['When rules are known and stable, or decisions must be explained', true],
          ['When there is a great deal of historical data', false],
          ['Never in a modern system; learning is always better', false],
          ['Only for problems small enough to enumerate', false]],
    ...
    checkpoint: [
      mcq('A product says it is "AI-powered". What have you learned?',
        [['Very little — it could be a language model or a set of if-statements', true],
          ['That it uses deep learning, which is what the term now implies', false],
          ['That it learns from its users and improves the more it is used', false],
          ['That it is state of the art, or the claim could not be made', false]],
      ...
      mcq('A spam filter written as hand-coded keyword rules is:',
        [['AI but not machine learning — nothing is learned from examples', true],
          ['Machine learning, because it classifies mail into two categories', false],
          ['Deep learning, since keyword matching is a form of pattern finding', false],
          ['Not AI at all, because a rule somebody wrote cannot be intelligent', false]],
    ...
    mcqs: [
      mcq('What does training a model actually do?',
        [['Adjusts numeric parameters to reduce the error on the training examples', true],
          ['Stores the examples so they can be matched later', false],
          ['Builds a set of rules covering the examples', false],
          ['Forms concepts in the way a person would', false]],
      ...
      mcq('A cat classifier fails on outdoor photos. The likely explanation is:',
        [['It learned something that separated its examples, such as the setting', true],
          ['The network needs more layers to generalise', false],
          ['Cats genuinely look different when photographed outdoors', false],
          ['A bug somewhere in the image loading code', false]],
      ...
      mcq('Why is performing well on the training data worthless by itself?',
        [['The model could simply have memorised it — generalisation is the actual goal', true],
          ['Training data is always wrong in some respect', false],
          ['Performing well on it proves the model overfitted', false],
          ['It is not worthless; it is the main measure used', false]],
      ...
      mcq('The requirement people most often skip is:',
        [['That the signal is present in the data at all', true],
          ['Having enough examples to learn the pattern', false],
          ['Having labels that are correct and consistent', false],
          ['Having enough computation to finish training', false]],
    ...
    checkpoint: [
      ...
      mcq('"We do not have the data to answer this" is:',
        [['A legitimate and frequently correct conclusion', true],
          ['An admission that the project has failed', false],
          ['Solved by gathering more data of the same kind', false],
          ['Solved by training a larger model on the data', false]],
    ...
    mcqs: [
      mcq('Predicting house price is:',
        [['Regression — the output is a quantity on a continuous scale', true],
          ['Classification, since the output is a house price', false], ['Unsupervised, because no labels are provided', false], ['Neither; price prediction is a separate category', false]],
      ...
      mcq('A model predicting "no disease" always is 99% accurate on a 1% disease. What does that show?',
        [['Accuracy alone is misleading when classes are imbalanced', true],
          ['That the model is performing very well indeed', false],
          ['That the data has been labelled incorrectly', false],
          ['That ninety-nine per cent is the best achievable', false]],
      ...
      mcq('Precision and recall differ how?',
        [['Precision: of those flagged, how many were right. Recall: how many it caught', true],
          ['They are the same measure under two different names', false],
          ['Precision applies to regression and recall to classes', false],
          ['Recall measures how quickly the model responds', false]],
      ...
      mcq('Why does label quality set a ceiling on model quality?',
        [['The labels define what correct means during training', true],
          ['Labels are expensive to produce in any quantity', false],
          ['More labels always produce a better model', false],
          ['It does not; a model can exceed its labels', false]],
    ...
    checkpoint: [
      mcq('For cancer screening, which matters more and why?',
        [['Recall — missing a real case is far worse than a false alarm', true],
          ['Precision, so that nobody is alarmed needlessly', false],
          ['Accuracy, taking both kinds of error together', false],
          ['They matter equally in a screening programme', false]],
      ...
      mcq('Predicting a 1-5 rating could be either task. What decides?',
        [['Whether 4 is "one more than 3" or merely a different category', true],
          ['The amount of data available for training decides between them', false],
          ['The model type chosen, which fixes the task it can perform', false],
          ['It is always classification, because there are five fixed values', false]],
...
to explore before deciding what to predict. It is frequently a first step rather than an answer.`,
    mcqs: [
      mcq('The defining difference from supervised learning is:',
        [['There are no labels, so there is no right answer to check', true],
          ['It uses considerably less data than supervised does', false],
          ['It is faster to train than a supervised model', false],
          ['It works only on numeric data and not on text', false]],
      ...
      mcq('You cluster customers into five groups. What has the algorithm established?',
        [['Nothing about whether five is right or whether the groups mean anything', true],
          ['That there are five real segments in the customers', false],
          ['That the data is well structured and clean enough', false],
          ['That five is the optimal number of groups to use', false]],
      ...
      mcq('How should clustering results be treated?',
        [['As hypotheses to be checked, not as findings', true],
          ['As confirmed segments of the customer base', false],
          ['As ground truth for a later supervised model', false],
          ['As a statistically significant finding about them', false]],
      ...
      mcq('When is unsupervised learning the right starting point?',
        [['You have data, no labels, and labelling is impractical', true],
          ['You already know what you are trying to predict', false],
          ['You have very little data to work with at all', false],
          ['Accuracy matters more than anything else does', false]],
    ...
    checkpoint: [
      mcq('How can you partially validate a clustering?',
        [['Check whether the clusters survive a rerun and mean something recognisable', true],
          ['Compute its accuracy against the labels the algorithm assigned itself', false],
          ['Compare it against labels, which is what makes clustering measurable', false],
          ['You cannot validate it at all, because there is nothing to compare with', false]],
      ...
      mcq('"We discovered five customer segments" is oversold because:',
        [['The number five was almost certainly chosen by a person', true],
          ['Five is too few segments for any real customer base', false],
          ['Customer segments are not real things and cannot be found', false],
          ['Clustering is unreliable and rarely produces the same answer', false]],
...
celebrating. Genuine 99% accuracy on a hard problem is rare; leakage producing it is common.`,
    mcqs: [
      mcq('Overfitting is:',
        [['Learning the training data so specifically that noise is captured instead of the pattern', true],
          ['Training the model on far too much data at once', false],
          ['Stopping the training run before it has converged', false],
          ['A model too simple to represent the real pattern', false]],
      ...
      mcq('Both training and test accuracy are poor. This indicates:',
        [['Underfitting — the model is too simple for the pattern', true],
          ['Overfitting, where training accuracy runs ahead', false],
          ['Data leakage from the test set into training', false],
          ['A good model, since the two figures agree closely', false]],
      ...
      mcq('Why must the test set be used only once?',
        [['Looking and then changing makes it a validation set, measuring nothing', true],
          ['Evaluating on it is computationally expensive', false],
          ['Repeated evaluation degrades the stored data', false],
          ['It need not be; the test set can be reused freely', false]],
    ...
    checkpoint: [
      mcq('Which is an example of data leakage?',
        [['Using "date account closed" as a feature for predicting account closure', true],
          ['Using 70% of the data for training and the remaining 30% for testing', false],
          ['Having more features in the table than there are rows to learn from', false],
          ['Training for many more epochs than the size of the data justifies', false]],
      ...
      mcq('Training accuracy rising while test accuracy falls indicates:',
        [['Overfitting — the gap between them is the diagnostic', true],
          ['Underfitting', false],
          ['Normal training, where the two curves separate as it learns', false],
          ['A bug, because test accuracy should track training accuracy', false]],
...
- How would we find out if it degraded?`,
    mcqs: [
      mcq('A model trained on historical hiring decisions is claimed to be objective. What is wrong?',
        [['It learned the past decisions, discriminatory ones included', true],
          ['It needs a great deal more historical data first', false],
          ['Nothing is wrong; a model has no opinions of its own', false],
          ['Hiring is not a process that can be modelled at all', false]],
      ...
      mcq('A model quietly gets worse over months with no code change. Most likely:',
        [['Distribution shift — the incoming data no longer resembles the training data', true],
          ['Overfitting', false],
          ['Hardware degradation on the machine serving it', false],
          ['A bug', false]],
      ...
      mcq('A pneumonia model partly learned to recognise a portable scanner. What failure is that?',
        [['Correlation captured as causation — it breaks in a different hospital', true],
          ['Overfitting to noise in the training images', false],
          ['Distribution shift', false],
          ['Data leakage from the labels into the features', false]],
    ...
    checkpoint: [
      mcq('A model is 99% confident and wrong. What does the confidence number mean?',
        [['Less than it appears: confidence is unreliable on unfamiliar inputs', true],
          ['There is a one per cent chance that this particular answer is wrong', false],
          ['The model is broken, because a correct model cannot be confidently wrong', false],
          ['The model is well calibrated, so its stated confidence can be trusted', false]],
      ...
      mcq('A loan model never learns about applicants it rejected because:',
        [['They never had the chance to repay, so no outcome exists', true],
          ['The data on rejected applicants is deleted after a year', false],
          ['Rejections are not recorded anywhere the model can see', false],
          ['Privacy rules forbid keeping data on a rejected applicant', false]],
    ...
    mcqs: [
      mcq('Predicting which of 26 letters a handwritten character is:',
        [['Supervised classification: labelled examples, categorical output', true],
          ['Supervised regression, predicting a numeric value', false],
          ['Unsupervised clustering into twenty-six groups', false],
          ['Anomaly detection, flagging unusual characters', false]],
      ...
      mcq('A resume screener trained on past hires is proposed. The first question is:',
        [['Whose resumes are under-represented in the historical hires?', true],
          ['How accurate is it on the historical hires?', false],
          ['Which model architecture has been chosen for it?', false],
          ['How fast can it screen a batch of applications?', false]],
      ...
      mcq('A model predicting equipment failure is 99.9% accurate. Failures are 0.1% of cases. What does that suggest?',
        [['It may simply be predicting "no failure" every time', true],
          ['That the model is performing excellently well', false],
          ['That the model has overfitted to the training set', false],
          ['That the failure data has been mislabelled', false]],
      ...
      mcq('Before proposing a model, you should deliberately:',
        [['Write down how it will fail before writing what it will achieve', true],
          ['Estimate the accuracy the model is likely to reach', false],
          ['Choose the architecture the model is going to use', false],
          ['Collect more data than the problem seems to need', false]],
    ...
    checkpoint: [
      mcq('A claim quotes accuracy on a heavily imbalanced problem. What should you ask for?',
        [['Precision and recall on the rare class', true],
          ['A larger sample from the same distribution', false],
          ['The architecture and how it was configured', false],
          ['How long the model took to train in total', false]],
      ...
      mcq('The most useful contribution a first-year can make to a model proposal is:',
        [['Asking what happens when it is wrong and who is under-represented in the data', true],
          ['Suggesting a better architecture than the one the team has chosen', false],
          ['Estimating what the training run is going to cost to complete', false],
          ['Recommending more training data, which almost always improves it', false]],
