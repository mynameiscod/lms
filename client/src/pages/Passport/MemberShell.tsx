      ...
      <aside className={`gd-side${mobileOpen ? ' open' : ''}`}>
        ...
        <nav className="gd-nav">
          ...
          {/*
            ...
            everybody, which is wrong for every second-year, whose plan is 110, and was already
            wrong for any tenant that had changed the Foundation length.
            THE FALLBACK IS THE OLD LABEL, DELIBERATELY. It reads while the dashboard is still
            loading and on any response that carries no length, and an unfamiliar "My Plan"
            appearing in those moments looks like the navigation has been renamed. Ninety is the
            default programme length, so the fallback is what it always was.
          */}
