...
programs.`,
    mcqs: [
      mcq('`find / -name "*.conf" > results.txt` still shows "Permission denied" on screen. Why?',
        [['Those messages are on stderr, and only stdout was redirected away', true],
          ['The redirection failed and nothing was written', false],
          ['`find` ignores redirection for its own messages', false],
          ['The output file filled up and stopped accepting', false]],
      ...
      mcq('How can you tell whether a message is on stdout or stderr?',
        [['Redirect stdout to /dev/null — anything still visible is stderr', true],
          ['Check the colour the terminal renders it in', false],
          ['An error message always begins with the word Error', false],
          ['You cannot tell them apart from the terminal alone', false]],
      ...
      mcq('What is exit status for?',
        [['So a program can tell whether the previous command succeeded', true],
          ['Reporting the error to whoever is at the terminal', false],
          ['Measuring how long the command took to finish', false],
          ['Counting how many lines of output were produced', false]],
    ...
    mcqs: [
      mcq('`command > file` where the command fails immediately leaves the file:',
        [['Empty — the shell truncates it before the command runs', true],
          ['Unchanged, since the command produced no output', false],
          ['Containing the error message the command produced', false],
          ['Deleted, because the command never wrote anything', false]],
      ...
      mcq('`cmd 2>&1 > file` sends stderr where?',
        [['To the terminal: it copied the old destination BEFORE stdout moved', true],
          ['To the file, alongside the ordinary output', false],
          ['To /dev/null, where it is discarded silently', false],
          ['Nowhere; the stream is closed by the redirection', false]],
      ...
      mcq('When is `2> /dev/null` risky?',
        [['In a script — silenced errors you never read hide a job failing for weeks', true],
          ['Never; errors you do not want are worth discarding', false],
          ['Only with `find`, which writes a great deal to stderr', false],
          ['When the disk is full and errors cannot be written', false]],
      ...
      mcq('Why prefer `>>` as a default habit?',
        [['`>` destroys the existing contents before the command even runs', true],
          ['`>>` is faster, since nothing has to be truncated', false],
          ['`>` is not supported by every shell in common use', false],
          ['`>>` creates the file where `>` requires it to exist', false]],
    ...
    checkpoint: [
      ...
      mcq('`sort < file` versus `sort file`:',
        [['They do the same thing, and the second is clearer', true],
          ['The first is faster, since the shell opens the file', false],
          ['The second does not work; sort reads only stdin', false],
          ['The first sorts the file in place and rewrites it', false]],
...
harmless and universal, and worth knowing about so you can read other people's pipelines.`,
    mcqs: [
      mcq('`ls | wc -l` counts files even though neither command knows about the other because:',
        [['Both speak the same interface: lines of text in and out', true],
          ['`ls` has a counting mode the shell is invoking', false],
          ['The shell counts the lines on behalf of both', false],
          ['`wc` understands the format a listing comes in', false]],
      ...
      mcq('The right way to build a four-stage pipeline is:',
        [['Add one stage at a time, looking at the output after each', true],
          ['Write the whole thing, then debug what comes out', false],
          ['Write it backwards, starting from the final output', false],
          ['Test each command on its own before joining them', false]],
      ...
      mcq('Errors from the middle of a pipeline go:',
        [['To your terminal — the pipe carries stdout only', true],
          ['Into the next command, alongside the output', false],
          ['To the last command in the pipeline, at the end', false],
          ['Are discarded, since a pipe carries only stdout', false]],
      ...
      mcq('Why can `head` make a pipeline over a huge file finish quickly?',
        [['Stages run concurrently, so head stopping can end the pipeline early', true],
          ['`head` reads the file backwards from the end', false],
          ['The shell optimises the pipeline before running it', false],
          ['It cannot; the whole file is read before head runs', false]],
    ...
    checkpoint: [
      ...
      mcq('`cat file | grep x` versus `grep x file`:',
        [['Identical output; the first uses one extra process', true],
          ['The first is faster, since `cat` reads ahead', false],
          ['The second cannot be used inside a longer pipeline', false],
          ['They differ once the file becomes large enough', false]],
...
Filtering early keeps later stages small and fast.`,
    mcqs: [
      ...
      mcq('Why quote a grep pattern?',
        [['Unquoted, the shell expands * and ? against filenames first', true],
          ['`grep` requires its pattern argument to be quoted', false],
          ['For readability, so the pattern stands out clearly', false],
          ['To allow a space to appear inside the pattern', false]],
      ...
      mcq('`grep "404"` on a web log matches more than you wanted. The best next step is:',
        [['Tighten the pattern with anchors or context, then look again', true],
          ['Add `-i` so the match is case-insensitive', false],
          ['Use `-c` to count the matches rather than show them', false],
          ['Pipe the output to `head` and read the first few', false]],
    ...
    checkpoint: [
      mcq('Exclude health-check noise from a log before filtering for errors. Which?',
        [['`grep -v healthcheck app.log | grep " 500 "`', true],
          ['`grep healthcheck app.log`, then read what remains', false],
          ['`grep -c " 500 " app.log`, counting them directly', false],
          ['`grep -l " 500 " app.log`, listing the file names', false]],
    ...
    mcqs: [
      mcq('Why is `uniq` almost always preceded by `sort`?',
        [['uniq only collapses ADJACENT duplicate lines', true],
          ['`uniq` refuses to run unless its input is sorted', false],
          ['`sort` removes the duplicates itself in any case', false],
          ['For speed, since sorted input is faster to scan', false]],
      ...
      mcq('`sort` without `-n` on a list of numbers gives:',
        [['Text ordering, so 10 sorts before 9', true],
          ['Correct numeric order, smallest value first', false],
          ['An error, since the input is not text to sort', false],
          ['An unpredictable order that changes each run', false]],
      ...
      mcq('What does `sort | uniq -c | sort -rn | head` do?',
        [['Ranks distinct lines by how often they occur, commonest first', true],
          ['Sorts the lines and removes the duplicate ones', false],
          ['Counts how many lines the input holds in total', false],
          ['Finds the longest lines and shows them first', false]],
      ...
      mcq('`cut -d\' \' -f2` on text with runs of multiple spaces behaves how?',
        [['Each space is a separator, so runs produce empty fields', true],
          ['A run of spaces is treated as a single separator', false],
          ['It reports an error about the ambiguous delimiter', false],
          ['It trims the repeated spaces away automatically', false]],
    ...
    mcqs: [
      mcq('Why quote `"$1"` rather than writing `$1`?',
        [['A filename containing a space would otherwise split into two arguments', true],
          ['Bash requires every parameter to be quoted', false],
          ['It is faster, since no splitting has to happen', false],
          ['To allow a numeric argument to be passed in', false]],
      ...
      mcq('`./script.sh` gives "Permission denied" on a file you can read. The cause is:',
        [['The execute bit is not set — chmod +x', true],
          ['The shebang line names an interpreter that is missing', false],
          ['The file is owned by root and you are not in its group', false],
          ['A syntax error on the first line of the script', false]],
      ...
      mcq('`set -u` protects against what?',
        [['A mistyped variable name silently becoming an empty string', true],
          ['Syntax errors elsewhere in the script body', false],
          ['A file named by the script not being present', false],
          ['A command taking far longer than it should', false]],
      ...
      mcq('Why send script errors to stderr and exit non-zero?',
        [['So the caller can separate output from errors and see the failure', true],
          ['It is conventional, and nothing actually depends on it', false],
          ['Writing to stderr is faster than writing to stdout', false],
          ['So the terminal shows the message in red', false]],
    ...
    checkpoint: [
      ...
      mcq('What does the shebang line do?',
        [['Tells the system which interpreter should run the file', true],
          ['Is a comment, and has no effect on how it runs', false],
          ['Sets the execute permission on the script file', false],
          ['Imports the standard bash function library', false]],
    ...
    mcqs: [
      mcq('Count how many DISTINCT IP addresses appear in field 1 of a log. Which?',
        [['`cut -d\' \' -f1 log | sort -u | wc -l`', true],
          ['`cut -d\' \' -f1 log | wc -l`, counting each line', false],
          ['`grep -c "\\." log`', false],
          ['`sort log | wc -l`', false]],
      ...
      mcq('`grep -c "error" file` returns 5, but you know there are 8 occurrences. Why?',
        [['`grep -c` counts matching LINES, and a line may match twice', true],
          ['`grep` missed three of them for some other reason', false],
          ['Case sensitivity excluded three of the occurrences', false],
          ['The file changed between the two times you looked', false]],
      ...
      mcq('Before writing any pipeline, the first step is:',
        [['Look at the first few lines of the data', true],
          ['Decide which of the commands the job will need', false],
          ['Count how many lines the input file contains', false],
          ['Write the script file that will hold the pipeline', false]],
      ...
      mcq('Your total exceeds the number of lines in the file. What does that suggest?',
        [['The pipeline is counting something other than what you think', true],
          ['The file grew while the pipeline was running', false],
          ['That is normal for a log file being written to', false],
          ['A rounding error somewhere in the arithmetic', false]],
    ...
    checkpoint: [
      ...
      mcq('`sort -rn` versus `sort -r`:',
        [['`-n` compares numerically; without it 9 sorts above 10', true],
          ['They are the same; `-n` is implied by `-r`', false],
          ['`-rn` reverses the order twice, cancelling out', false],
          ['`-r` already sorts numerically by default', false]],
