 ...
 * The certified units, their content and quizzes are read and never edited.
 *
...
(async () => {
  ...
  check('pre', `units 359, READY 354, PUBLISHED ${certified.length}`, fpBefore.total === 359 && ready.units.length === 354 && fpBefore.published === certified.length,
    `${fpBefore.total} / ${ready.units.length} / ${fpBefore.published}`);
  check('pre', `PRODUCTION inventory is exactly the certified ${certified.length}`, JSON.stringify(prodCodes) === JSON.stringify(certified), `${prodCodes.length}`);
  ...
  check('activation', `PRODUCTION inventory is still exactly the certified ${certified.length}`, JSON.stringify(prodAfter) === JSON.stringify(certified));
  ...
  check('final', `PUBLISHED ${certified.length} and PRODUCTION inventory exactly the certified set`, fpAfter.published === certified.length && JSON.stringify(prodFinal) === JSON.stringify(certified));
