...
export const CAREER_SKILL_TAXONOMY: SeedSkill[] = [
  ...
  // ══ Year 3: depth, a specialization, and proof somebody can be hired ═════════════════════
  //
  // Year 2 taught the engineering skills; Year 3 takes them to production depth and points them
  // at one direction. Everything here is INTERMEDIATE or ADVANCED on purpose: nothing in this
  // block is a first encounter, and each one names something an interviewer can probe.
  //
  // Where a Year-2 skill already covers the ground it is REUSED rather than duplicated.
  // CODE_REVIEW, PORTFOLIO_EVIDENCE, INTERNSHIP_READINESS, SYSTEM_DESIGN_BASICS,
  // TECHNICAL_INTERVIEW_PREP, CLOUD_FUNDAMENTALS and DEVOPS_FUNDAMENTALS all stay as they are;
  // Year 3 asks more of them through depth and target level rather than through a second key.

  // ── Advanced programming and design ──
  { key: 'DESIGN_PATTERNS', name: 'Design Patterns', parentKey: 'PROGRAMMING',
    difficulty: 'ADVANCED', displayOrder: 200,
    description: 'The handful that earn their place - strategy, factory, observer, adapter - and when not to reach for one.',
    aliases: ['Patterns'], prerequisiteKeys: ['OOP_CONCEPTS'] },
  { key: 'REFACTORING', name: 'Refactoring', parentKey: 'PROGRAMMING',
    difficulty: 'ADVANCED', displayOrder: 210,
    description: 'Changing the shape of working code without changing what it does, in steps that stay green.',
    prerequisiteKeys: ['CLEAN_CODE', 'TESTING_FUNDAMENTALS'] },
  { key: 'DEPENDENCY_MANAGEMENT', name: 'Dependency Management', parentKey: 'PROGRAMMING',
    difficulty: 'INTERMEDIATE', displayOrder: 220,
    description: 'Versions, lockfiles, transitive dependencies, and what taking on a library commits you to.',
    aliases: ['Package Management'] },
  { key: 'ERROR_HANDLING_DESIGN', name: 'Error Handling Design', parentKey: 'PROGRAMMING',
    difficulty: 'ADVANCED', displayOrder: 230,
    description: 'Deciding what fails loudly, what retries, what degrades, and what the caller is told.',
    prerequisiteKeys: ['PYTHON_ERROR_HANDLING'] },

  // ── Advanced data structures and algorithms ──
  { key: 'DSA_HEAPS', name: 'Heaps & Priority Queues', parentKey: 'DSA',
    difficulty: 'ADVANCED', displayOrder: 130,
    description: 'Keeping the next-most-important item cheap to reach, and the problems that shape solves.',
    prerequisiteKeys: ['DSA_TREES'] },
  { key: 'DSA_DIVIDE_CONQUER', name: 'Divide & Conquer', parentKey: 'DSA',
    difficulty: 'ADVANCED', displayOrder: 150,
    description: 'Splitting a problem until it is trivial, and the recurrence that says what it cost.',
    prerequisiteKeys: ['DSA_RECURSION', 'DSA_COMPLEXITY'] },
  { key: 'ALGORITHM_DESIGN', name: 'Algorithm Design', parentKey: 'DSA',
    difficulty: 'ADVANCED', displayOrder: 160,
    description: 'Choosing an approach against real constraints, and arguing why it fits before writing it.',
    prerequisiteKeys: ['DSA_COMPLEXITY'] },

  // ── Database and data engineering ──
  { key: 'QUERY_OPTIMIZATION', name: 'Query Optimization', parentKey: 'DATABASES',
    difficulty: 'ADVANCED', displayOrder: 100,
    description: 'Reading a query plan and knowing which change actually moved the number.',
    prerequisiteKeys: ['DB_INDEXING', 'SQL_JOINS'] },
  { key: 'NOSQL_CONCEPTS', name: 'NoSQL Concepts', parentKey: 'DATABASES',
    difficulty: 'INTERMEDIATE', displayOrder: 110,
    description: 'Document and key-value stores, what they give up for what, and when relational is still right.' },
  { key: 'CACHING', name: 'Caching', parentKey: 'DATABASES',
    difficulty: 'ADVANCED', displayOrder: 120,
    description: 'What is safe to cache, for how long, and how it goes stale. Invalidation is the hard half.',
    prerequisiteKeys: ['DB_INDEXING'] },
  { key: 'DATA_PIPELINES', name: 'Data Pipelines', parentKey: 'DATABASES',
    difficulty: 'ADVANCED', displayOrder: 130,
    description: 'Moving data between systems repeatedly and correctly, including what happens on a rerun.',
    prerequisiteKeys: ['DATA_WRANGLING'] },
  { key: 'DATA_VISUALIZATION', name: 'Data Visualization', parentKey: 'DATABASES',
    difficulty: 'INTERMEDIATE', displayOrder: 140,
    description: 'Choosing a chart that answers the question asked, and not one that flatters the number.',
    prerequisiteKeys: ['DATA_WRANGLING'] },

  // ── Web and API engineering ──
  { key: 'API_DESIGN', name: 'API Design', parentKey: 'WEB_FUNDAMENTALS',
    difficulty: 'ADVANCED', displayOrder: 100,
    description: 'Resources, versioning, pagination, filtering and errors - the decisions callers live with.',
    prerequisiteKeys: ['REST_APIS'] },
  { key: 'AUTHENTICATION', name: 'Authentication', parentKey: 'WEB_FUNDAMENTALS',
    difficulty: 'ADVANCED', displayOrder: 110,
    description: 'Proving who is calling: sessions, tokens, expiry, and where each one is kept.',
    aliases: ['Auth'], prerequisiteKeys: ['HTTP'] },
  { key: 'AUTHORIZATION', name: 'Authorization', parentKey: 'WEB_FUNDAMENTALS',
    difficulty: 'ADVANCED', displayOrder: 120,
    description: 'Deciding what a proven caller may do. The check that is missed most often.',
    aliases: ['Access Control'], prerequisiteKeys: ['AUTHENTICATION'] },
  { key: 'WEB_SECURITY', name: 'Web Security', parentKey: 'WEB_FUNDAMENTALS',
    difficulty: 'ADVANCED', displayOrder: 130,
    description: 'Injection, XSS, CSRF and the input-validation habit that prevents most of them.',
    prerequisiteKeys: ['SECURITY_FUNDAMENTALS'] },
  { key: 'STATE_MANAGEMENT', name: 'State Management', parentKey: 'WEB_FUNDAMENTALS',
    difficulty: 'ADVANCED', displayOrder: 140,
    description: 'Where a value lives, who may change it, and why a screen showed the wrong number.',
    prerequisiteKeys: ['JS_ASYNC'] },
  { key: 'RESPONSIVE_DESIGN', name: 'Responsive Design', parentKey: 'WEB_FUNDAMENTALS',
    difficulty: 'INTERMEDIATE', displayOrder: 150,
    description: 'One interface that works on a phone and on a monitor without two codebases.',
    prerequisiteKeys: ['CSS'] },

  // ── Software engineering at production depth ──
  { key: 'SOFTWARE_ARCHITECTURE', name: 'Software Architecture', parentKey: 'SE_PRACTICES',
    difficulty: 'ADVANCED', displayOrder: 90,
    description: 'Layers, boundaries and separation of concerns - the decisions that are expensive to reverse.',
    prerequisiteKeys: ['SYSTEM_DESIGN_BASICS'] },
  { key: 'TECHNICAL_DEBT', name: 'Technical Debt', parentKey: 'SE_PRACTICES',
    difficulty: 'ADVANCED', displayOrder: 95,
    description: 'Naming the cost of a shortcut, deciding when to take one, and when to pay it back.',
    prerequisiteKeys: ['REFACTORING'] },
  { key: 'LOGGING_DIAGNOSTICS', name: 'Logging & Diagnostics', parentKey: 'SE_PRACTICES',
    difficulty: 'INTERMEDIATE', displayOrder: 100,
    description: 'Leaving enough behind that a failure at 3am can be explained without reproducing it.',
    aliases: ['Logging'] },
  { key: 'MONITORING_OBSERVABILITY', name: 'Monitoring & Observability', parentKey: 'SE_PRACTICES',
    difficulty: 'ADVANCED', displayOrder: 110,
    description: 'Knowing a system is unwell before a user reports it, and which signal said so.',
    prerequisiteKeys: ['LOGGING_DIAGNOSTICS'] },
  { key: 'AUTOMATED_TESTING', name: 'Automated Testing', parentKey: 'SE_PRACTICES',
    difficulty: 'ADVANCED', displayOrder: 120,
    description: 'Unit, integration and boundary tests that run unattended and fail for one reason.',
    prerequisiteKeys: ['TESTING_FUNDAMENTALS'] },
  { key: 'CI_CD', name: 'CI/CD', parentKey: 'SE_PRACTICES',
    difficulty: 'ADVANCED', displayOrder: 130,
    description: 'A pipeline that builds, tests and ships the same way every time, without a person remembering.',
    aliases: ['Continuous Integration'],
    prerequisiteKeys: ['AUTOMATED_TESTING', 'GIT_BRANCHING'] },
  { key: 'CONTAINERS_DOCKER', name: 'Containers & Docker', parentKey: 'SE_PRACTICES',
    difficulty: 'ADVANCED', displayOrder: 140,
    description: 'Packaging an application with what it needs so it runs the same off your machine.',
    aliases: ['Docker'], prerequisiteKeys: ['SHELL_COMMANDS'] },
  { key: 'LINUX_ADMINISTRATION', name: 'Linux Administration', parentKey: 'SE_PRACTICES',
    difficulty: 'INTERMEDIATE', displayOrder: 150,
    description: 'Services, processes, ports, SSH and logs on a machine nobody has a desktop on.',
    prerequisiteKeys: ['SHELL_PIPELINES', 'OS_PROCESSES'] },
  { key: 'DEPLOYMENT', name: 'Deployment', parentKey: 'SE_PRACTICES',
    difficulty: 'ADVANCED', displayOrder: 160,
    description: 'Getting a build in front of real users, and getting it back off when it is wrong.',
    prerequisiteKeys: ['CONTAINERS_DOCKER'] },
  { key: 'PRODUCTION_ENGINEERING', name: 'Production Engineering', parentKey: 'SE_PRACTICES',
    difficulty: 'ADVANCED', displayOrder: 170,
    description: 'Carrying one project from requirements to something deployed, tested and explainable.',
    prerequisiteKeys: ['DEPLOYMENT', 'AUTOMATED_TESTING'] },

  // ── Security engineering ──
  { key: 'THREAT_MODELING', name: 'Threat Modelling', parentKey: 'CS_FUNDAMENTALS',
    difficulty: 'ADVANCED', displayOrder: 70,
    description: 'Asking what an attacker wants and where the system would let them have it.',
    prerequisiteKeys: ['SECURITY_FUNDAMENTALS'] },
  { key: 'SECURE_CODING', name: 'Secure Coding', parentKey: 'CS_FUNDAMENTALS',
    difficulty: 'ADVANCED', displayOrder: 80,
    description: 'Validating input, managing secrets and handling dependencies as a routine habit.',
    prerequisiteKeys: ['WEB_SECURITY'] },

  // ── Applied machine learning ──
  { key: 'ML_WORKFLOW', name: 'ML Workflow', parentKey: 'AI_LITERACY',
    difficulty: 'ADVANCED', displayOrder: 60,
    description: 'Data to features to a train/validation/test split, and why the split comes first.',
    prerequisiteKeys: ['AI_ML_CONCEPTS'] },
  { key: 'ML_EVALUATION', name: 'ML Evaluation', parentKey: 'AI_LITERACY',
    difficulty: 'ADVANCED', displayOrder: 70,
    description: 'Whether a model is any good, what overfitting looks like, and which metric to believe.',
    prerequisiteKeys: ['ML_WORKFLOW', 'PROBABILITY_STATISTICS'] },
  { key: 'FEATURE_ENGINEERING', name: 'Feature Engineering', parentKey: 'AI_LITERACY',
    difficulty: 'ADVANCED', displayOrder: 80,
    description: 'Turning raw columns into something a model can use, without leaking the answer.',
    prerequisiteKeys: ['ML_WORKFLOW'] },

  // ── Career proof ──
  { key: 'TECHNICAL_WRITING', name: 'Technical Writing', parentKey: 'PROFESSIONAL_SKILLS',
    difficulty: 'INTERMEDIATE', displayOrder: 100,
    description: 'Documentation, a design note, a decision record - writing another engineer can act on.',
    prerequisiteKeys: ['TECHNICAL_COMMUNICATION'] },
  { key: 'BEHAVIORAL_INTERVIEW', name: 'Behavioural Interviews', parentKey: 'PROFESSIONAL_SKILLS',
    difficulty: 'INTERMEDIATE', displayOrder: 110,
    description: 'Telling what you did, why, and what you would change - with a real example ready.',
    prerequisiteKeys: ['TECHNICAL_EXPLANATION'] },
];
