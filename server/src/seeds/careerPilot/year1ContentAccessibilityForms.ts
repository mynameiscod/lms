...
reach for when no correct element exists.`,
    mcqs: [
      mcq('Which best describes who accessibility work serves?',
        [['People with permanent, temporary and situational impairments', true],
          ['Blind users specifically, who depend on a screen reader', false],
          ['A small minority whose needs are met by a separate page', false],
          ['Users of assistive technology, and nobody else on the site', false]],
      ...
      mcq('Why is accessibility cheap to build in and expensive to retrofit?',
        [['Retrofitting means the markup is already wrong, so the fix is structural', true],
          ['The specialist tooling is expensive to license and to learn', false],
          ['It requires specialists, who are scarce and charge accordingly', false],
          ['It is not expensive to retrofit; the work is the same either way', false]],
      ...
      mcq('Building only to satisfy a legal requirement produces:',
        [['Minimum compliance, which is different from a page that works', true],
          ['Full accessibility, since the standards were written for it', false],
          ['The cheapest outcome, which is the point of a legal floor', false],
          ['The same result as building for usability from the start', false]],
    ...
    mcqs: [
      mcq('What does `<button>` give you that `<div onclick>` does not?',
        [['Focus, Enter and Space activation, an announced role, and disabled handling', true],
          ['Styling that works without a class name on the element', false],
          ['A faster click, since no extra handler has to be dispatched', false],
          ['Nothing', false]],
      ...
      mcq('"The first rule of ARIA is not to use ARIA" means:',
        [['Use a native element wherever one exists; ARIA is for what the platform lacks', true],
          ['ARIA is deprecated and the native elements have replaced it', false],
          ['ARIA is only for experts, and beginners should leave it alone', false],
          ['It is a joke among specialists with no real advice behind it', false]],
      ...
      mcq('The one-minute semantics test is:',
        [['Turn off CSS and check the page still reads as a sensible document', true],
          ['Run an automated checker over the page and read its report', false],
          ['Tab through the page and check every control is reachable', false],
          ['Zoom the page to 200% and check nothing is cut off', false]],
    ...
    mcqs: [
      mcq('Tab order follows:',
        [['The DOM order, which CSS can make differ from the visual layout', true],
          ['The visual layout, top to bottom and then left to right', false],
          ['The tabindex values alone, and the DOM order is not consulted', false],
          ['Alphabetical order of each control\u2019s accessible name', false]],
      ...
      mcq('`tabindex="1"` should be used:',
        [['Never: it jumps ahead of everything and breaks the page order', true],
          ['On the first control, so the user starts where you intend', false],
          ['On custom widgets, which have no natural place in the order', false],
          ['Inside modals, so focus lands on the dialog when it opens', false]],
      ...
      mcq('A keyboard trap is:',
        [['Focus that enters something and cannot leave, forcing a page reload', true],
          ['A tab order so long that reaching the end takes a while', false],
          ['Too many focusable elements for a user to tab through', false],
          ['A missing skip link, so the navigation is traversed each time', false]],
    ...
    mcqs: [
      mcq('How does a screen reader user usually navigate a page?',
        [['By jumping between headings, landmarks, links and form fields', true],
          ['By listening to the whole page from the top downwards', false],
          ['By searching the page for the text they are looking for', false],
          ['By tabbing through every control the page contains in turn', false]],
      ...
      mcq('`alt=""` on an image means:',
        [['Decorative — skip it. Different from omitting alt entirely', true],
          ['The alt text is missing and the author intended to add it', false],
          ['The image failed to load and has nothing to describe itself', false],
          ['It is an error; alt must always describe what is in the image', false]],
      ...
      mcq('A screen reader lists the links on a page and eleven of them read "read more". What has been lost?',
        [['The links-list mode, which needs each link to name its destination', true],
          ['The tab order, which now visits the eleven links in sequence', false],
          ['The heading structure, which the links were standing in for', false],
          ['Nothing; the surrounding text still explains each of them', false]],
    ...
    checkpoint: [
      ...
      mcq('The most effective way to find screen-reader problems in your own page is:',
        [['Turn one on and try to complete a task with your eyes closed', true],
          ['Run an automated checker, which reports what a reader will say', false],
          ['Read the ARIA specification and check the page against it', false],
          ['Ask a colleague to try it, since a fresh reader notices more', false]],
...
matter disproportionately for dyslexic readers and for anybody reading at length.`,
    mcqs: [
      ...
      mcq('Why use `rem` rather than `px` for text?',
        [['rem respects a changed default font size, and px overrides it', true],
          ['rem is the newer unit and px is retained for compatibility', false],
          ['px is deprecated for text and kept only for borders now', false],
          ['There is no practical difference once the page is zoomed', false]],
      ...
      mcq('Placeholder text used as a label fails because:',
        [['It is usually low contrast and disappears as soon as the user types', true],
          ['It is too short to describe the field it is standing in for', false],
          ['Screen readers ignore it entirely, so the field is unnamed', false],
          ['It is fine, provided the placeholder repeats the label text', false]],
    ...
    mcqs: [
      ...
      mcq('Which can automation NOT determine?',
        [['Whether the alt text actually conveys what the image conveys', true],
          ['Whether an alt attribute is present on every image on the page', false],
          ['The contrast ratio between the text and its background', false],
          ['Which form fields have no label associated with them', false]],
      ...
      mcq('Which manual test finds the most problems?',
        [['Completing a real task with a screen reader, eyes closed', true],
          ['Tabbing through every control and watching the focus ring', false],
          ['Zooming to 200% and checking nothing is cut off the page', false],
          ['Viewing the page in greyscale to check colour is not the only cue', false]],
      ...
      mcq('A green Lighthouse accessibility score means:',
        [['The obvious mistakes are absent — not that the page is usable', true],
          ['The page is accessible to somebody using assistive technology', false],
          ['The page meets WCAG AA across all of its success criteria', false],
          ['No manual testing is needed beyond what the tool has done', false]],
    ...
    checkpoint: [
      ...
      mcq('`role="tablist"` on something that ignores arrow keys is:',
        [['Worse than no role — it promises behaviour that is not there', true],
          ['Better than nothing, since the role at least describes the intent', false],
          ['Neutral: the role is ignored by anything that cannot use it', false],
          ['Caught by automated tools, which check roles against behaviour', false]],
...
ARIA before semantics.`,
    mcqs: [
      ...
      mcq('Why run the automated checker LAST?',
        [['Run first, it directs attention to what it measures, not what matters', true],
          ['It is slow, and running it early wastes time on a changing page', false],
          ['It gives false positives that have to be triaged out by hand', false],
          ['It should be run first, so the cheap problems are gone early', false]],
      ...
      mcq('Why say who a problem affects?',
        [['"Keyboard users cannot reach the menu" gets prioritised; "accessibility issue" does not', true],
          ['It is required in an accessibility report by most audit standards', false],
          ['It makes the finding sound more serious than it might otherwise', false],
          ['It is not especially useful; the fix is the same either way', false]],
      ...
      mcq('Adding ARIA that duplicates native behaviour is:',
        [['A common over-correction from learning ARIA before semantics', true],
          ['Best practice, since it states the intent twice for safety', false],
          ['Harmless and worth doing wherever the role can be inferred', false],
          ['Required for compliance, which asks for explicit roles', false]],
    ...
    mcqs: [
      mcq('`type="email"` gives you what beyond text input?',
        [['A mobile keyboard with @, browser validation, and autofill', true],
          ['Nothing practical beyond documenting the intent', false],
          ['Validation performed by the server on submit', false],
          ['Encryption of the value before it is sent', false]],
      ...
      mcq('Three mutually exclusive options are best presented as:',
        [['Radio buttons — all visible, one click', true],
          ['A select, which takes up less vertical space', false],
          ['Checkboxes, one for each of the three options', false],
          ['A text input, with the options named beside it', false]],
      ...
      mcq('An input with no `name` attribute:',
        [['Submits nothing and reports no error, as if the field were empty', true],
          ['Fails validation when the form is submitted', false],
          ['Cannot be focused with the keyboard at all', false],
          ['Is submitted under the key "unnamed" instead', false]],
    ...
    checkpoint: [
      mcq('A group of radio buttons needs a `<fieldset>` and `<legend>` so that:',
        [['A screen reader announces the question the options belong to', true],
          ['The whole group can be styled consistently with one selector', false],
          ['Only one of the options can be selected at any given time', false],
          ['All of the options submit together as a single form value', false]],
...
attribute, which the browser and assistive technology both understand.`,
    mcqs: [
      mcq('Why is a placeholder not a substitute for a label?',
        [['It disappears when typing, is low contrast, and is read inconsistently', true],
          ['It is too short to describe the field properly', false],
          ['It cannot be styled the way a label can be', false],
          ['The placeholder attribute has been deprecated', false]],
      ...
      mcq('Hiding a label with `display: none` is wrong because:',
        [['It is removed from the accessibility tree, so nothing announces it', true],
          ['It breaks the layout around the input field', false],
          ['It is slow for the browser to recalculate', false],
          ['It is fine, since the field is still labelled', false]],
      ...
      mcq('Clicking a label focuses its input. Why does that matter most for checkboxes?',
        [['The checkbox itself is a very small target, especially on a phone', true],
          ['A checkbox cannot be focused any other way', false],
          ['It is required for a checkbox to validate', false],
          ['It does not matter especially for a checkbox', false]],
    ...
    checkpoint: [
      mcq('A search field shows only a magnifying glass. The correct approach is:',
        [['A real label, hidden with a visually-hidden class', true],
          ['A placeholder reading "Search" inside the box', false],
          ['No label; the icon already says what it is', false],
          ['A label hidden with `display: none` in the CSS', false]],
      ...
      mcq('Required fields should be marked:',
        [['In text as well as visually, and with the `required` attribute', true],
          ['With a red asterisk, which everybody recognises as required', false],
          ['With a bold label, so the required fields stand out from the rest', false],
          ['Not at all; validation will tell the user which ones were needed', false]],
...
validate everything again**, with no exceptions.`,
    mcqs: [
      mcq('Why prefer built-in validation over JavaScript?',
        [['Messages are translated, behaviour is familiar, and it works early', true],
          ['It is more secure than a script the user can edit', false],
          ['JavaScript cannot validate a form reliably at all', false],
          ['It executes faster than the equivalent script', false]],
      ...
      mcq('`input:invalid` styling is a problem because:',
        [['It matches on page load, so required fields are red before anyone types', true],
          ['The pseudo-class is not supported in every browser', false],
          ['It overrides the other styles on the input', false],
          ['It is not a problem; red is expected on invalid', false]],
      ...
      mcq('An overly strict email `pattern` causes:',
        [['Legitimate addresses to be rejected', true],
          ['Validation to run more slowly than it should', false],
          ['A security hole an attacker could make use of', false],
          ['Nothing; a strict pattern is always safer', false]],
      ...
      mcq('Client-side validation provides:',
        [['Helpfulness — the server must validate everything again', true],
          ['Security against data the user has tampered with', false],
          ['Both security and helpfulness, in equal measure', false],
          ['Protection against malformed data reaching storage', false]],
    ...
    checkpoint: [
      mcq('You need a password-confirmation check. What should you do?',
        [['Extend the built-in validation with JavaScript, using checkValidity', true],
          ['Disable the built-in validation and write your own', false],
          ['Use a `pattern` attribute on the second field', false],
          ['Validate the match on the server and nowhere else', false]],
...
form data?" prompt. Post, then redirect to a GET.`,
    mcqs: [
      mcq('Which is the right method for a search form, and why?',
        [['GET: the URL then describes the results and can be shared', true],
          ['POST, so the search query stays out of the URL', false],
          ['Either one; there is no practical difference', false],
          ['POST, which is the more modern of the two', false]],
      ...
      mcq('A "Clear" button inside a form unexpectedly submits it. Why?',
        [['A button with no type defaults to submit inside a form', true],
          ['"Clear" is a reserved button name in HTML', false],
          ['The form has no action attribute set on it', false],
          ['Every button inside a form submits it by design', false]],
      ...
      mcq('Why never send a password over GET?',
        [['It appears in the address bar, history, logs and the Referer header', true],
          ['A GET request is slower than a POST is', false],
          ['GET has a length limit a password may exceed', false],
          ['A GET request cannot be encrypted by TLS', false]],
      ...
      mcq('After a successful POST you should:',
        [['Redirect to a GET, so a refresh does not re-submit', true],
          ['Render the result page directly from the POST', false],
          ['Reload the form so the fields are cleared', false],
          ['Rely on the back button to leave the page', false]],
    ...
    mcqs: [
      mcq('The three questions an error message must answer are:',
        [['What is wrong, which field, and what to do about it', true],
          ['What went wrong, when it happened, and why', false],
          ['The field, an error code, and how severe it is', false],
          ['The error, its cause, and an id for the log', false]],
      ...
      mcq('When should a field-level error usually appear?',
        [['On blur, when the user leaves the field', true],
          ['While the user is still typing into the field', false],
          ['Only when the whole form is finally submitted', false],
          ['As soon as the page loads, before anything is typed', false]],
      ...
      mcq('What makes an error announced to a screen reader?',
        [['aria-describedby on the input pointing at the message element', true],
          ['A red border drawn around the offending field', false],
          ['The word "error" appearing in the message text', false],
          ['A `role="alert"` on the message, and nothing else', false]],
      ...
      mcq('A stale error on a now-corrected field is:',
        [['Worse than no error: it says the form is wrong when it is not', true],
          ['Harmless, since the user knows what they fixed', false],
          ['Useful as a reminder of what went wrong before', false],
          ['Expected behaviour until the form is resubmitted', false]],
    ...
    checkpoint: [
      mcq('On submit with errors, focus should move to:',
        [['The first field with an error', true],
          ['The top of the page, where a summary sits', false],
          ['The submit button, ready for another attempt', false],
          ['Nowhere; focus should be left where it was', false]],
      ...
      mcq('Which message is most useful?',
        [['"Use DD/MM/YYYY, for example 14/03/2006"', true],
          ['"The date format you entered is invalid"', false],
          ['"There is an error in the date field"', false],
    ...
    mcqs: [
      mcq('Why is a search form best submitted with GET?',
        [['The URL then describes the results and can be bookmarked and shared', true],
          ['It is faster than a POST for the same query', false],
          ['A POST would fail on a search endpoint', false],
          ['Search queries are short enough for a URL', false]],
      ...
      mcq('Which awkward input is most likely to break a name field?',
        [['An apostrophe, as in O\'Brien', true],
          ['An unusually long email address', false],
          ['A date a long way in the past', false],
          ['A number where a name was expected', false]],
      ...
      mcq('The recovery path worth testing is:',
        [['Submit wrongly, fix one field, and check the others persist', true],
          ['Submit the form correctly two times in a row', false],
          ['Refresh the page and see what has been kept', false],
          ['Clear the form and start filling it in again', false]],
      ...
      mcq('A multi-step form must survive:',
        [['The back button, without losing what was entered', true],
          ['Only navigation forwards through the steps', false],
          ['A page refresh, and nothing else besides', false],
          ['Nothing in particular beyond the happy path', false]],
    ...
    checkpoint: [
      mcq('Every value taken from a form should first be:',
        [['Trimmed, and converted if it needs to be a number', true],
          ['Uppercased, so that comparisons later on are case-insensitive', false],
          ['Escaped for HTML, before it goes anywhere near the database', false],
          ['Stored as it arrived, so the original input is never lost', false]],
      ...
      mcq('Which of these is a server responsibility rather than a client one?',
        [['Validating that the data is actually acceptable', true],
          ['Showing the error message beside the field', false],
          ['Choosing the right input type for each field', false],
          ['Managing where the focus goes after submit', false]],
