/**
 * The rules the Year-3 golden bank holds to.
 *
...
const skills = [...new Set(YEAR3_BANK.map(i => i.skillKey))];
const bySkill = (k: string) => YEAR3_BANK.filter(i => i.skillKey === k);

describe('the bank is structurally sound', () => {
  it('gives every question a unique id', () => {
    const seen = new Map<string, number>();
    for (const i of YEAR3_BANK) seen.set(i.questionId, (seen.get(i.questionId) || 0) + 1);
...
describe('the answer is not guessable from its shape', () => {
  /**
   * The same rule the Year-3 content holds. A correct option markedly longer than every
  ...
  it('keeps the answer-length tell at or below one in ten', () => {
    const share = YEAR3_BANK.filter(tells).length / (YEAR3_BANK.length || 1);
  ...
  it('spreads the correct answer across all four positions', () => {
    const counts = [0, 1, 2, 3].map(n => YEAR3_BANK.filter(i => present(i).correctIndex === n).length);
    const share = Math.max(...counts) / (YEAR3_BANK.length || 1);
