...
export function scoreAttempt(
  ...
  // Pathway: honor the student's stated goal; else infer from technical strength.
  const goal = (ctx.careerGoal || '').toLowerCase();
  let pathway = 'it_bridge';
  if (goal.includes('data')) pathway = 'data_analytics';
  else if (goal.includes('ai')) pathway = 'ai_ready';
  else if (goal.includes('software') || goal.includes('develop')) pathway = 'software_dev';
  else {
    const tech = categoryScores.find(c => c.key === 'technical')?.score || 0;
    pathway = tech >= 50 ? 'software_dev' : 'it_bridge';
  }
  ...
  return {
    ...
    pathway, pathwayLabel: PATHWAYS[pathway] || 'IT Career Bridge',
    weekPreview,
  };
}
