    ...
    mcqs: [
      mcq('Two servers both average 200ms. What can you conclude?',
        [['Very little — the spread could be tiny or enormous', true],
          ['That the two servers perform identically in practice', false],
          ['That both of them are performing acceptably well', false],
          ['That one of the two is faster than the other', false]],
      ...
      mcq('Description and inference differ how?',
        [['Description summarises what you have; inference claims what you do not', true],
          ['Description is used on small datasets and inference on large', false],
          ['Inference is the more accurate of the two approaches', false],
          ['They are the same activity under two different names', false]],
      ...
      mcq('For API latency, which is usually more useful than the mean?',
        [['A high percentile, such as p95 or p99', true],
          ['The mode, which is the commonest observed value', false], ['The range, from the fastest to the slowest request', false], ['The total time spent serving every request', false]],
      ...
      mcq('The most broadly valuable use of statistics for a developer is:',
        [['Reading somebody else\'s claim critically', true],
          ['Building predictive models from collected data', false],
          ['Computing averages over the data you gather', false],
          ['Drawing charts that present the data clearly', false]],
    ...
    checkpoint: [
      ...
      mcq('Where do most statistical errors occur?',
        [['In inference — claims made beyond the data collected', true],
          ['In the arithmetic, which is where a slip is easiest to make', false],
          ['In the charting, where the axes decide what the reader sees', false],
          ['In data entry, since one mistyped value moves every summary', false]],
...
information worth reporting.`,
    mcqs: [
      ...
      mcq('Why prefer the median for response times?',
        [['They are skewed, with a floor and no ceiling, so the mean is pulled up', true],
          ['The median is easier to compute than the mean is', false],
          ['The mean is not a valid summary for a time measure', false],
          ['Response times are always symmetric about the centre', false]],
      ...
      mcq('The mode is the only average that works for:',
        [['Categories, such as the most common browser', true],
          ['Skewed numeric data, where the mean misleads', false],
          ['Large datasets, where the other two are costly', false],
          ['Time series, where the order of values matters', false]],
      ...
      mcq('Mean and median are far apart. What does that tell you?',
        [['The data is skewed, and the gap is itself worth reporting', true],
          ['One of the two was computed incorrectly somewhere', false],
          ['The sample is too small', false],
          ['Nothing useful; the two often differ by chance', false]],
    ...
    checkpoint: [
      mcq('A company reports mean salary 113k and median 37k. What is happening?',
        [['A small number of very high earners are pulling the mean up', true],
          ['An arithmetic error, since the two should be close together', false],
          ['Most people earn about 75k, which is halfway between the two', false],
          ['The sample is too small', false]],
...
p99 2s" says exactly what happens without assuming a shape the data does not have.`,
    mcqs: [
      mcq('Why report standard deviation rather than variance?',
        [['SD is in the original units and variance in units squared', true],
          ['The standard deviation is the more accurate measure', false],
          ['Variance is harder to compute than the deviation', false],
          ['They are the same quantity under two names', false]],
      ...
      mcq('Mean 200ms, SD 350ms. What does that indicate?',
        [['Highly variable data where the mean says little', true],
          ['Consistently good performance across the requests', false],
          ['An arithmetic error, since SD cannot exceed the mean', false],
          ['That most of the requests take around 350ms', false]],
      ...
      mcq('For heavily skewed response times, what is more honest than mean and SD?',
        [['Percentiles such as p50, p95 and p99', true],
          ['The range, from the fastest to the slowest', false],
          ['The mode, or commonest response time seen', false],
          ['A larger sample, gathered over a longer period', false]],
    ...
    checkpoint: [
      mcq('An average reported without any measure of spread is:',
        [['An incomplete statement — it cannot distinguish steady from wildly variable', true],
          ['Sufficient for most purposes, provided the sample is reasonably large', false],
          ['Wrong, because a mean on its own can always be misread', false],
          ['Fine once the sample is large, since spread shrinks as n grows', false]],
      ...
      mcq('Why does the coefficient of variation exist?',
        [['To compare variability between quantities measured on different scales', true],
          ['To replace the standard deviation, which is harder to interpret', false],
          ['To handle skew, where the mean and median pull in opposite directions', false],
          ['To compute percentiles without having to sort the whole dataset', false]],
    ...
    mcqs: [
      mcq('Response times peak at 50ms and again at 3s. What does that suggest?',
        [['Two populations mixed together — likely cache hits and misses', true],
          ['Bad measurement somewhere in the collection', false],
          ['A normal distribution with unusually wide spread', false],
          ['A single outlier pulling the tail out to 3s', false]],
      ...
      mcq('Right-skewed data has:',
        [['Most values low with a long high tail, and mean above median', true],
          ['Most of the values high, with a long low tail', false],
          ['Two peaks, one on each side of the centre', false],
          ['A mean that sits below the median value', false]],
      ...
      mcq('"Anything beyond three standard deviations is an outlier" assumes:',
        [['A normal distribution — it is simply wrong for skewed data', true],
          ['A sample large enough for the rule to apply', false],
          ['Numeric data rather than categories or labels', false],
          ['Nothing at all; it holds for any distribution', false]],
      ...
      mcq('Why look at the shape before computing summaries?',
        [['Every summary assumes a shape, and describes nothing if that fails', true],
          ['It is faster than computing the summaries first', false],
          ['To find the errors in the data before summarising', false],
          ['It is not necessary; the summaries hold regardless', false]],
    ...
    checkpoint: [
      mcq('Data is not normally distributed. What follows?',
        [['Nothing is wrong — most real data is not normal', true],
          ['The data is faulty and should be collected again', false],
          ['A larger sample is needed before it can be used', false],
          ['It cannot be analysed with standard techniques', false]],
      ...
      mcq('The cheapest first step with an unfamiliar dataset is:',
        [['Plot a histogram, or look at the quartiles', true],
          ['Compute the mean of every numeric column', false],
          ['Remove the outliers so the rest is readable', false],
...
constantly at scale: a million requests a day means one every day.`,
    mcqs: [
      mcq('A fair coin lands heads five times. P(heads) next toss is:',
        [['0.5 — the coin has no memory', true], ['Less than 0.5, since tails is now overdue', false], ['More than 0.5, since heads is clearly favoured', false], ['Unknowable without more tosses to learn from', false]],
    ...
    checkpoint: [
      mcq('Your redundancy plan multiplies two servers\' failure probabilities. What is the risk?',
        [['They are not independent, so the real joint probability is much higher', true],
          ['The arithmetic is wrong; probabilities are added, not multiplied', false],
          ['Probabilities cannot be multiplied together under any circumstances', false],
          ['No risk at all, provided both figures were measured correctly', false]],
      ...
      mcq('Hash collisions occur sooner than expected because of:',
        [['The birthday argument — pairs grow quadratically', true],
          ['Poorly designed hash functions in common use', false],
          ['Insufficient entropy in the values being hashed', false],
          ['Bugs in the implementation of the hash table', false]],
...
means. You need the base rate too.`,
    mcqs: [
      ...
      mcq('A 99%-accurate anomaly detector on 99.9% benign traffic produces:',
        [['Overwhelmingly false alerts, which teams then rationally ignore', true],
          ['Alerts that can be relied on almost all of the time', false],
          ['Alerts that are correct ninety-nine times in a hundred', false],
          ['Too few alerts, so real anomalies are missed', false]],
      ...
      mcq('Which question should you ask of a positive result?',
        [['Of everybody with this result, how many have the condition?', true],
          ['How accurate is the test across all the cases seen?', false],
          ['What sample size was the test originally validated on?', false],
          ['Who made the test, and what interest do they have?', false]],
    ...
    checkpoint: [
      mcq('A spam filter becomes useless when spam drops from 10% to 1% of mail. Why?',
        [['False positives from the far larger clean group now outnumber the true ones', true],
          ['The filter degrades over time as spammers change their wording', false],
          ['There is less spam to train on, so the model learns less from it', false],
          ['It does not; a filter that worked at ten per cent still works', false]],
      ...
      mcq('A practical fix when detecting something rare is:',
        [['Narrow the population tested, raising the base rate within that group', true],
          ['Increase the sample size until the rare group is large enough', false],
          ['Report overall accuracy instead, which stays reassuringly high', false],
          ['Accept the false positives as the unavoidable cost of detection', false]],
    ...
    mcqs: [
      mcq('Returning aircraft show damage on wings and tail. Where should armour go?',
        [['The engines: planes hit there did not return to be counted', true],
          ['The wings and the tail, where the damage is', false],
          ['Everywhere equally, since any part can be hit', false],
          ['The cockpit, which is the most critical part', false]],
      ...
      mcq('Ice cream sales correlate with drownings because:',
        [['Hot weather causes both — it is a confounder', true],
          ['Eating ice cream makes people more likely to drown', false],
          ['It is a coincidence with no explanation at all', false],
          ['People who swim tend to buy ice cream afterwards', false]],
      ...
      mcq('"Sales up 40%" is most suspicious because:',
        [['The baseline is unstated and may have been chosen', true],
          ['A forty per cent rise is implausibly large', false],
          ['Sales are not something that can be measured', false],
          ['It is not accompanied by a chart of the figures', false]],
      ...
      mcq('"Engagement up 30%" may be bad news because:',
        [['Higher engagement can mean users cannot find what they need', true],
          ['Engagement is not something that can be measured', false],
          ['A thirty per cent rise is implausibly large', false],
          ['A rise in engagement is always a bad sign', false]],
    ...
    checkpoint: [
      mcq('A survey shows 90% satisfaction, taken via a pop-up to current users. The flaw is:',
        [['Dissatisfied users had already left and were never asked', true],
          ['The sample is too small to support a figure as precise as 90%', false],
          ['Pop-up surveys are unreliable because they interrupt the user', false],
          ['There is no flaw; asking current users is exactly the point', false]],
      ...
      mcq('Before reading the shape of a chart, look at:',
        [['The axes — especially whether the y-axis starts at zero', true],
          ['The title, which states what the chart is claiming to show', false],
          ['The colour scheme, which decides what the eye is drawn to first', false],
          ['The source, since who produced the chart shapes what it shows', false]],
    ...
    mcqs: [
      mcq('"70% preferred B" with no sample size. What should you conclude?',
        [['Nothing yet; a percentage without a denominator is unsupported', true],
          ['That option B is genuinely the preferred one', false],
          ['That the sample must have been reasonably large', false],
          ['That the figure is roughly right even so', false]],
      ...
      mcq('The commonest flaw in an otherwise correct analysis is:',
        [['The conclusion does not follow from the numbers', true],
          ['Arithmetic errors somewhere in the calculation', false],
          ['The wrong chart type for the data being shown', false],
          ['A sample too small to support the conclusion', false]],
      ...
      mcq('When is excluding outliers legitimate?',
        [['When the rule was decided before seeing which points it removes', true],
          ['Whenever they distort the mean beyond usefulness', false],
          ['Never; every observation belongs in the analysis', false],
          ['When a value lies beyond three standard deviations', false]],
      ...
      mcq('The most useful single action when auditing an analysis is:',
        [['Recompute one number yourself from the raw data', true],
          ['Redraw the chart from the underlying figures', false],
          ['Check the citations the analysis depends upon', false],
          ['Increase the sample size and run it all again', false]],
    ...
    checkpoint: [
      mcq('An average falls outside the range of the data. What does that mean?',
        [['An arithmetic or data-handling error — it is impossible', true],
          ['The data is heavily skewed in one direction', false],
          ['Some outliers were removed before averaging', false],
          ['It is possible when the mean has been weighted', false]],
      ...
      mcq('"Crime up 20%" after the recording definition changed is:',
        [['Not a like-for-like comparison, so it does not show an increase', true],
          ['A real increase, since the same force recorded both of the figures', false],
          ['A sampling problem, because only reported crimes are ever counted', false],
          ['A correlation between the new definition and the recorded total', false]],
    ...
    mcqs: [
      mcq('Response times: 45, 48, 51, 47, 49, 3200 ms. Which summary is honest?',
        [['Median 48.5 with the outlier reported separately', true],
          ['The mean of 573ms, which uses every value', false],
          ['The mean with its standard deviation beside it', false],
          ['The range of 3155ms, from lowest to highest', false]],
      ...
      mcq('A dataset has mean 50 and SD 2. What can you say about a value of 70?',
        [['It is ten SDs out: extremely unusual, and possibly an error', true],
          ['It is normal variation for a spread of this size', false],
          ['Nothing at all without seeing more of the data', false],
          ['It is exactly two standard deviations from the mean', false]],
      ...
      mcq('Group A improved 10% and group B 12%, from 50 people each. What follows?',
        [['Possibly nothing — the difference may be noise at this sample size', true],
          ['That group B is better than group A on this measure', false],
          ['That group B did twenty per cent better than group A', false],
          ['That whatever group A received is inadequate', false]],
      ...
      mcq('Reporting a standard deviation without saying what it means is:',
        [['Half an answer — interpretation is the point', true],
          ['Complete, since the number speaks for itself', false],
          ['Acceptable for a technical audience who know it', false],
          ['Better than reporting no measure of spread at all', false]],
      ...
      mcq('Your computed mean is far from your eyeball estimate. Most likely:',
        [['The computation used the wrong column or missed a filter', true],
          ['Your intuition about the data was simply wrong', false],
          ['The data is skewed, which the eye does not allow for', false],
          ['Rounding applied at some step of the calculation', false]],
    ...
    checkpoint: [
      mcq('The first step with an unfamiliar dataset is:',
        [['Look at the shape — it decides which summary is meaningful', true],
          ['Compute the mean, which is the summary every dataset supports', false],
          ['Remove the outliers, so they do not distort what follows', false],
          ['Plot a trend line, which shows where the data is heading', false]],
