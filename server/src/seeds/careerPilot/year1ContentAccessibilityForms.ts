    ...
    checkpoint: [
      ...
      mcq('Landmarks such as `<main>` and `<nav>` allow a user to:',
        [['Jump directly to a region instead of hearing the navigation on every page', true],
          ['Style regions more easily', false],
          ['Improve load time', false],
          ['Nothing functional', false]],
...
always get back out? That single test finds most keyboard problems.`,
    ...
    checkpoint: [
      mcq('What must a well-built modal do for keyboard users?',
        [['Take focus on open, close on Escape, cycle Tab within itself, and return focus on close', true],
          ['Only close on Escape', false],
          ['Nothing special', false],
          ['Set tabindex="1" inside', false]],
      ...
      mcq('The thirty-second keyboard test is:',
        [['Tab through the page: reach everything, see focus, activate, and get back out', true],
          ['Run an automated checker', false],
          ['Test with a screen reader', false],
          ['Zoom to 200%', false]],
...
ARIA before semantics.`,
    ...
    checkpoint: [
      mcq('A modal never receives focus when opened. Who is affected and how?',
        [['Keyboard and screen-reader users, who have no indication it opened or way to reach it', true],
          ['Nobody', false],
          ['Mouse users', false],
      ...
      mcq('After making a fix, the next step is:',
        [['Re-test that specific thing', true],
          ['Move to the next issue', false],
          ['Run the full suite', false],
          ['Commit', false]],
    ...
    checkpoint: [
      ...
      mcq('Why not disable the submit button until the form is valid?',
        [['Disabled controls are skipped by the keyboard and give no reason why', true],
          ['It is slower', false],
          ['Disabled is deprecated', false],
          ['It is good practice', false]],
    ...
    checkpoint: [
      ...
      mcq('`pattern` needs `title` because:',
        [['The title supplies the message explaining what the pattern requires', true],
          ['It is required syntax', false],
          ['It sets the placeholder', false],
          ['It enables the pattern', false]],
...
form data?" prompt. Post, then redirect to a GET.`,
    ...
    checkpoint: [
      ...
      mcq('An input without a `name` attribute results in:',
        [['That value never reaching the server, with no error', true],
          ['A validation failure', false],
          ['A console warning', false],
          ['The id being used instead', false]],
