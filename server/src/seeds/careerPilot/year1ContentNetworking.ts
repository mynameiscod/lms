...
Learning the chain is learning the diagnostic order, which is what the tools unit builds on.`,
    mcqs: [
      mcq('Why is networking organised in layers?',
        [['Each layer offers a service and hides how, so the one above ignores it', true],
          ['To make the subject easier to teach in a syllabus', false],
          ['For security, since each layer can be secured alone', false],
          ['To allow more addresses than one layer could hold', false]],
      ...
      mcq('A user reports "it does not work". Why is the chain the useful framing?',
        [['Six different failures present identically, and each has a different fix', true],
          ['It is faster to explain to the person reporting it', false],
          ['Users understand the layers and can report which failed', false],
          ['It identifies the cause automatically from the symptom', false]],
    ...
    checkpoint: [
      mcq('Your browser works identically on wifi and on mobile data because:',
        [['Lower layers hide the medium from the ones above', true],
          ['Both connections use the same IP address for you', false],
          ['HTTP detects the connection type and adapts to it', false],
          ['The server adapts its responses to the medium', false]],
      ...
      mcq('In the chain, what comes immediately after resolving the name?',
        [['Connecting to the address on a particular port', true],
          ['Encrypting the connection with the server', false],
          ['Sending the HTTP request', false],
          ['Routing the packets towards the destination', false]],
...
256 addresses. Machines in the same subnet talk directly; anything else goes via the gateway.`,
    mcqs: [
      mcq('Your laptop has 192.168.1.10. What does that tell you?',
        [['It is a private address, unique only on your local network', true],
          ['It is publicly reachable from anywhere on the internet', false],
          ['It is an IPv6 address written in a shortened form', false],
          ['It is unique worldwide and belongs only to you', false]],
      ...
      mcq('Why does an inbound connection to your laptop fail by default?',
        [['NAT has no record saying which internal machine it belongs to', true],
          ['Firewalls block every inbound connection by default', false],
          ['A laptop is not able to accept inbound connections', false],
          ['The internet provider forbids inbound connections', false]],
      ...
      mcq('A service works in development and not in Docker. A likely networking cause is:',
        [['It binds to 127.0.0.1 rather than 0.0.0.0', true],
          ['The container image is too large to start properly', false],
          ['DNS is not configured inside the container', false],
          ['The port is already in use by another process', false]],
    ...
    checkpoint: [
      mcq('Many devices in a home share one public IP because of:',
        [['NAT translating outbound connections and tracking replies', true],
          ['IPv6, which gives every device its own routable address', false], ['DNS, which resolves each device back to the same name', false], ['Subnetting, which divides the public address between them', false]],
      ...
      mcq('`/24` in CIDR notation means:',
        [['The first 24 bits identify the network, leaving 256 addresses', true],
          ['That the network has room for twenty-four machines on it', false],
          ['Version 24 of the addressing protocol is in use on this network', false],
          ['A twenty-four second timeout before the route is given up on', false]],
    ...
    mcqs: [
      mcq('You change a DNS record and it does not take effect. Most likely cause?',
        [['Resolvers are still serving the cached old answer until the TTL expires', true],
          ['The record is invalid and was silently rejected', false],
          ['The nameserver holding the zone is unreachable', false],
          ['Propagation inherently takes a day or two to finish', false]],
      ...
      mcq('When should you lower a TTL before a planned change?',
        [['In advance; lowering it during the change helps nobody already caching', true],
          ['At the moment the change is made, along with it', false],
          ['Afterwards, once the new record has settled', false],
          ['It makes no difference when the TTL is lowered', false]],
      ...
      mcq('`dig example.com +short` returns nothing but `curl` to the IP works. Conclusion?',
        [['The server is fine and name resolution is broken', true],
          ['The server is down and not answering any requests', false],
          ['A firewall is blocking the connection to the server', false],
          ['The certificate on the server has expired', false]],
      ...
      mcq('A CNAME record holds:',
        [['An alias pointing at another name', true],
          ['An IPv4 address for the name being looked up', false],
          ['The mail server that handles that domain', false],
          ['Arbitrary text, used for verification records', false]],
    ...
    checkpoint: [
      mcq('The highest-value first diagnostic when a site will not load is:',
        [['Check whether the name resolves, separately from anything else', true],
          ['Restart the server, which clears whatever state has gone wrong', false],
          ['Check the certificate, since an expired one blocks the page', false],
          ['Check the firewall, which is the commonest cause of a failure', false]],
      ...
      mcq('Why is "DNS propagation" a misleading term?',
        [['Nothing is pushed — cached answers simply expire at different times', true],
          ['Resolution is instant, so there is nothing to propagate at all', false],
          ['It only applies to A records, and not to any other record type', false],
          ['It is not misleading; the new record really is pushed outwards', false]],
    ...
    mcqs: [
      mcq('"Connection refused" immediately means:',
        [['You reached the machine and nothing was listening on that port', true],
          ['A firewall dropped the packet before it arrived', false],
          ['The address you connected to is the wrong one', false],
          ['The machine you are connecting to is switched off', false]],
      ...
      mcq('A connection attempt that times out suggests:',
        [['Something dropped the packet silently — a firewall, or the wrong address', true],
          ['The service is not running on the target machine', false],
          ['The port is closed and refusing the connection', false],
          ['A certificate problem stopping the TLS handshake', false]],
      ...
      mcq('Why can thousands of clients use port 443 on one server simultaneously?',
        [['A connection is identified by source address and port as well', true],
          ['The server opens a further port for each client', false],
          ['Connections are queued and served one at a time', false],
          ['Each connection is given a slice of time in turn', false]],
      ...
      mcq('Development servers default to 3000 or 8080 because:',
        [['Ports below 1024 require root on Unix', true],
          ['Those ports are faster than the low-numbered ones', false],
          ['Port 80 is reserved for browsers and cannot be used', false],
          ['Convention alone, with no technical reason at all', false]],
    ...
    checkpoint: [
      mcq('Before blaming the network for an unreachable service, check:',
        [['What is actually listening, with ss -tlnp or netstat', true],
          ['The DNS record, since the name may point at the wrong machine', false],
          ['The certificate, which blocks the connection once expired', false],
          ['The route to the host, which may be going somewhere else', false]],
      ...
      mcq('Refused and timed out point at different fixes because:',
        [['Refused means you reached the machine; timed out means nothing came back at all', true],
          ['They are the same failure reported with two different wordings', false],
          ['Refused always means a firewall, and timed out always means the app', false],
          ['A timeout means the service crashed while handling your request', false]],
...
better decisions about it.`,
    mcqs: [
      mcq('Why does a video call use UDP?',
        [['Retransmitting stale audio is worse than losing it; late data is useless', true],
          ['UDP is more secure, since there is no handshake', false],
          ['TCP cannot carry audio or video data at all', false],
          ['UDP uses less bandwidth for the same amount of data', false]],
      ...
      mcq('The TCP three-way handshake costs:',
        [['A round trip before any data moves', true],
          ['Nothing measurable on a modern fast connection', false],
          ['Three full round trips before any data can move', false],
          ['Extra bandwidth, but no additional delay at all', false]],
      ...
      mcq('The general rule for choosing is:',
        [['Late data useless means UDP; missing data wrong means TCP', true],
          ['Always use TCP, unless raw speed is the priority', false],
          ['Use UDP whenever the data being sent is small', false],
          ['Use TCP for anything that genuinely matters', false]],
      ...
      mcq('HTTP/3 uses UDP in order to:',
        [['Avoid one lost packet blocking unrelated streams, as TCP does by design', true],
          ['Be faster to implement than a new TCP variant', false],
          ['Reduce the cost of encrypting each connection', false],
          ['Support older networks that predate modern TCP', false]],
    ...
    checkpoint: [
      mcq('TCP retransmits a lost packet. What happens to data sent after it?',
        [['It waits — head-of-line blocking delays everything behind the lost packet', true],
          ['It is delivered immediately, and the missing packet is filled in later', false],
          ['It is retransmitted too, because the window restarts from the loss', false],
          ['It is discarded, and the sender begins again from the lost packet', false]],
      ...
      mcq('DNS uses UDP primarily because:',
        [['A query and reply are small and a retry is cheaper than a handshake', true],
          ['DNS data is unimportant enough that a lost reply does not matter', false],
          ['UDP is encrypted by default, and DNS queries reveal what you visit', false],
          ['TCP cannot carry DNS at all, so there is no alternative available', false]],
...
compromised server is an encrypted connection to a compromised server.`,
    mcqs: [
      mcq('What does a valid certificate prove?',
        [['That the holder controls that domain name — nothing more', true],
          ['That the organisation behind the site is legitimate', false],
          ['That the site is safe to enter your details into', false],
          ['That the content of the site has been verified', false]],
      ...
      mcq('A "name mismatch" certificate warning means:',
        [['The certificate is for a different domain: a misconfiguration, or worse', true],
          ['The certificate has passed its expiry date', false],
          ['The issuing authority is not in the trust store', false],
          ['The connection was established without encryption', false]],
      ...
      mcq('Why is a self-signed certificate acceptable in development and not from a stranger?',
        [['Nobody vouched for it — you trust it only because you created it', true],
          ['It uses weaker encryption than a purchased one', false],
          ['It expires sooner than one from an authority', false],
          ['It is not acceptable anywhere, including locally', false]],
    ...
    checkpoint: [
      mcq('A site shows a padlock. What can a user conclude?',
        [['The connection is private and matches the name shown, and nothing more', true],
          ['The site is safe, because a certificate authority has vetted its owner', false],
          ['The company behind the site has been verified as a real legal entity', false],
          ['The content is accurate, since the site has been checked before issue', false]],
      ...
      mcq('Why is clicking through certificate warnings a serious habit?',
        [['Usually it is a misconfiguration, and the once it is not is the time that matters', true],
          ['It is slow, because the browser re-checks the certificate each time', false],
          ['It disables encryption for that site until the browser cache is cleared', false],
          ['It is not serious, since the warning is almost always a misconfigured clock', false]],
    ...
    mcqs: [
      mcq('`ping` gets no reply. What can you conclude?',
        [['Very little — many hosts block ICMP, so failure is inconclusive', true],
          ['That the host is down and not answering anything', false],
          ['That name resolution is broken somewhere upstream', false],
          ['That the port you wanted is closed on that host', false]],
      ...
      mcq('`curl` reports a 500 error. What does that tell you about the network?',
        [['It is entirely fine — the whole chain worked and the application failed', true],
          ['A firewall is interfering somewhere along the path', false],
          ['The DNS record is misconfigured for that hostname', false],
          ['The certificate presented by the server is invalid', false]],
      ...
      mcq('"Connection refused" versus "connection timed out":',
        [['Refused means you reached the machine; timed out means packets vanished', true],
          ['They are equivalent messages from different tools', false],
          ['Refused means the name could not be resolved at all', false],
      ...
      mcq('Why is `curl -v` the most useful single command here?',
        [['It narrates resolution, connection, TLS and response, and where it stopped', true],
          ['It is the fastest of the diagnostic tools available', false],
          ['It works even when name resolution is unavailable', false],
          ['It bypasses a firewall that is blocking other tools', false]],
    ...
    checkpoint: [
      mcq('It fails from your machine and works from a colleague\'s. What does that establish?',
        [['The problem is local — your DNS, firewall or VPN', true],
          ['The server is answering intermittently to everybody', false],
          ['A routing problem somewhere upstream of you both', false],
          ['Nothing at all; the two machines may differ anyway', false]],
      ...
      mcq('You should stop diagnosing at:',
        [['The first link in the chain that fails', true],
          ['The last link, once every other one has been checked', false],
          ['The application layer, where most faults turn out to be', false],
          ['After every link in the chain has been checked once', false]],
    ...
    mcqs: [
      mcq('You connect to a site by IP with curl and get a certificate warning. Why?',
        [['The certificate is issued for the name, and you did not use the name', true],
          ['The certificate expired a short while ago', false],
          ['A connection made by IP address cannot use TLS', false],
          ['The issuing authority is not trusted by your machine', false]],
      ...
      mcq('A service is listening but unreachable from another machine. Check first:',
        [['Which interface it is bound to — 127.0.0.1 versus 0.0.0.0', true],
          ['The DNS record, in case the name is now stale', false],
          ['The certificate, which may have expired overnight', false],
          ['The route packets take between the two machines', false]],
      ...
      mcq('Before running any command, you should:',
        [['State what you expect to happen at each link', true],
          ['Check the logs on the machine you are connecting to', false],
          ['Restart the service and see whether that clears it', false],
          ['Ping the gateway to check the local network works', false]],
      ...
      mcq('The output says "connection timed out". The next step is:',
        [['Investigate what might be dropping packets: firewall, group, address', true],
          ['Restart the application and try the connection again', false],
          ['Renew the certificate, which may have just expired', false],
          ['Check the DNS record still points at the right host', false]],
      ...
      mcq('Why compare from a second machine?',
        [['It separates a local problem from a genuine one in seconds', true],
          ['To measure the latency between the two machines', false],
          ['To test whether name resolution works elsewhere', false],
          ['It is not useful; two machines share the same network', false]],
    ...
    checkpoint: [
      mcq('Diagnosis should proceed:',
        [['Along the chain in order, stopping at the first failure', true],
          ['From the application backwards towards the network', false],
          ['By checking every link, then deciding what failed', false],
          ['By restarting each component until it starts working', false]],
      ...
      mcq('Which finding proves the network is fine?',
        [['An HTTP 500 response from the application', true],
          ['A ping that receives a reply from the server', false],
          ['A DNS name that resolves to the expected address', false],
          ['A port that accepts a connection when tested', false]],
