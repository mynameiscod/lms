    ...
    mcqs: [
      mcq('What does a program do when an `if` condition is false and there is no `else`?',
        [['Skips the indented block and carries on with the next unindented line', true],
          ['Stops the program, since no branch was able to run', false],
          ['Runs the indented block once anyway before moving on', false],
          ['Raises an error, because every if needs a matching else', false]],
      ...
      mcq('Why does Python reject `if score = 10:`?',
        [['`=` assigns a value; a condition needs a comparison, which is `==`', true],
          ['A condition cannot mention a bare number on either side', false],
          ['`score` has to be a boolean before it can appear in an if', false],
          ['The colon belongs after the value rather than the condition', false]],
...
order. If it matches something other than the top branch, the order is wrong.`,
    mcqs: [
      ...
      mcq('Why is `elif marks >= 75` unreachable after `if marks >= 50`?',
        [['Every value satisfying >= 75 already satisfies >= 50, so the first claims it', true],
          ['An `elif` cannot follow a branch that used a `>=` test', false],
          ['75 is greater than 50, so the order has been reversed', false],
          ['It is reachable, for any mark between 75 and 100', false]],
      ...
      mcq('Replacing `elif` with a second `if` changes the behaviour how?',
        [['Both tests are evaluated, so a later assignment can overwrite an earlier one', true],
          ['No change at all; `elif` is shorthand for a nested if', false],
          ['The second test becomes unreachable once the first passes', false],
          ['It raises a SyntaxError, since the chain is now broken', false]],
      ...
      mcq('For the A/B/C/F chain above, which input proves the ordering is correct?',
        [['A high value like 90 — it must reach the top branch, not a lower one', true],
          ['0, because it has to come out as an F and nothing else', false],
          ['A negative number, which no branch should ever claim', false],
          ['Any value at all, since ordering cannot be tested directly', false]],
...
Short-circuiting is the whole reason the order is not arbitrary.`,
    mcqs: [
      mcq('`if day == "Sat" or "Sun":` — why is this always true?',
        [['It parses as `(day == "Sat") or ("Sun")`, and "Sun" is truthy', true],
          ['`or` is not allowed between two string values in Python', false],
          ['It compares day against both values, which is what was meant', false],
          ['It raises a TypeError, which is being swallowed somewhere', false]],
      ...
      mcq('Why does `if n != 0 and total / n > 5:` never divide by zero?',
        [['`and` short-circuits: a false left side means the right side is never evaluated', true],
          ['Python catches the division error silently inside the `and`', false],
          ['Division by zero returns 0, which is then compared with 5', false],
          ['It does divide by zero whenever n happens to be zero', false]],
    ...
    mcqs: [
      mcq('When is nesting genuinely the right structure?',
        [['When the inner condition is meaningless unless the outer one is true', true],
          ['Whenever two conditions both have to be satisfied', false],
      ...
      mcq('What does a guard clause do to the main path of a function?',
        [['Leaves it unindented at the bottom, after the exceptional cases have returned', true],
          ['Wraps the main path in a try block with the guards', false],
          ['Moves the main path into a nested else at the bottom', false],
          ['Removes the error handling that the nesting provided', false]],
      ...
      mcq('Before flattening a three-level condition, the safe first step is:',
        [['Enumerate every combination of outcomes as a table', true],
          ['Rename the variables so each condition reads clearly', false],
          ['Add a comment beside each branch saying what it means', false],
    ...
    mcqs: [
      ...
      mcq('Why is `if quantity:` a risky test for an order quantity?',
        [['A legitimate quantity of 0 is falsy and would be silently skipped', true],
          ['Integers cannot be used in a condition without a comparison', false],
          ['It raises a TypeError when the quantity happens to be 0', false],
          ['It is not risky; an order quantity is never going to be 0', false]],
      ...
      mcq('Why prefer `if x is None:` over `if x == None:`?',
        [['None is a singleton, so identity is the exact question being asked', true],
          ['`==` does not work with None and raises a TypeError', false],
          ['`is` is quicker to type and reads more like English', false],
          ['They differ only in style, and either one is acceptable', false]],
    ...
    checkpoint: [
      ...
      mcq('`if flag == True:` behaves differently from `if flag:` when flag is:',
        [['"yes" — truthy, but not equal to True', true],
          ['True, where the two tests disagree about the value', false],
          ['False, where equality and truthiness come apart', false],
          ['They never differ; the two forms are exactly equivalent', false]],
...
print(grade(int(input())))`,
    ...
    mcqs: [
      mcq('A condition produces the wrong answer but no error. What does that tell you?',
        [['The expression is valid Python and the logic is wrong, so no traceback', true],
          ['The interpreter is faulty and should be reinstalled', false],
          ['There is a missing import that would have caught it', false],
          ['The variable is undefined and is silently treated as empty', false]],
      ...
      mcq('`age = input("Age: ")` then `if age >= 18:` raises a TypeError. Why?',
        [['`input()` returns a string, and a string cannot be compared to an integer with >=', true],
          ['`age` is undefined at the point where it is compared', false],
          ['The 18 must be quoted before it can be compared', false],
          ['The prompt string is too long for the input function', false]],
      ...
      mcq('The best first debugging action for a misbehaving compound condition is:',
        [['Print each operand separately to see which one is not what you assumed', true],
          ['Flip the operator and run it again to see what changes', false],
          ['Delete the else branch so only one path can be taken', false],
          ['Add a try/except around the condition being evaluated', false]],
    ...
    mcqs: [
      ...
      mcq('Which correctly tests that a year is a leap year?',
        [['`(year % 4 == 0 and year % 100 != 0) or year % 400 == 0`', true],
          ['`year % 4 == 0`, which is the rule most people remember', false],
    ...
    checkpoint: [
      mcq('You have written a chain and one branch never runs for any input. The cause is almost always:',
        [['A looser condition placed above a stricter one', true],
          ['A missing else, so the last case has nowhere to go', false],
          ['Wrong indentation, nesting one branch inside another', false],
          ['The branch is unnecessary and should simply be removed', false]],
