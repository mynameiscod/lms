...
>
> **Change log (2026-07-13, polish):** **Branded email template.** Cold/follow-up/reply emails now render through a redesigned `brandedHtml` (navy CodeBegun header + tagline, teal accent, properly-spaced paragraphs, "•" bullet lists, a styled signature block, compliant footer) instead of the bare plain-text `'light'` mode. Copy rewritten (warmer, structured intro with a skills list + clear ask). ⚠️ **Deliverability note:** the sending domain (codebegun.com) was flagged by Hostinger for **missing SPF/DKIM/DMARC** — fix these DNS records or outreach may land in spam regardless of design.
>
